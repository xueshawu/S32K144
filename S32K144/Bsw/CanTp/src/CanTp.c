...
#define MAX_SEGMENT_DATA_SIZE		CANTP_LL_DL 	// Size of a CAN frame data bytes.
...
static void sendFlowControlFrame(const CanTp_RxNSduType *rxConfig, CanTp_ChannelPrivateType *rxRuntime, BufReq_ReturnType flowStatus) {
	...
	case BUFREQ_E_BUSY:
		...
		pduInfo.SduLength = indexCount;
		break;
	case BUFREQ_E_OVFL: /** @req CANTP081 */
...
static void handleFirstFrame(const CanTp_RxNSduType *rxConfig,
	...
	if(0 == pduLength)
	{
		pduLength = ((uint32)data[2]<<24) + ((uint32)data[3]<<16) + ((uint32)data[4]<<8) + ((uint32)data[5]);
		data = &data[6];
	}
	else
	{
