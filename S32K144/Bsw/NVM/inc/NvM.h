...
/*  1.0.1            1. 增加Crc 校验错误接口
 *                   2. 增加写错误接口
 *                   3. 增加版本信息接口
 *  
 *  1.0.2           1.	CRC错误值上报 功能已实现，
                    2.	同时修复了冗余Block无法利用WriteAll写入的Bug,
                    3.	冗余Blcok 只上报一次crc错误计数
...
extern boolean NvM_GetIsCrcInvalid(void);
extern boolean NvM_GetIsWriteErr(void);
extern void NvM_SetFirstUseFlag(boolean flg);
extern boolean NvM_GetFirstUseFlag(void);
...
Std_ReturnType NvM_WriteBlock( NvM_BlockIdType blockId, const void *srcPtr );	/** @req NVM455 */
