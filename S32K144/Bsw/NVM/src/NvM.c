...
/*
#pragma section all "ASWRAM_Immdt"
volatile uint8  Asw_Immdt_testDt = 1;
#pragma section all restore
*/



volatile static NvmStateType 				nvmState = NVM_UNINITIALIZED;
...
static uint32                       nvmCrcErrorCnt = 0;
static uint32                       nvmWriteErrorCnt = 0;
static boolean                     nvmbFirstUsed = FALSE;
//static int nvmSetNr;
static AdministrativeBlockType 		AdminBlock[NVM_NUM_OF_NVRAM_BLOCKS];
static AdministrativeMultiBlockType AdminMultiBlock;

static uint32 nvmBlockCrcVal[NVM_NUM_OF_NVRAM_BLOCKS][2] = {
	{0,0},{0,0},{0,0},{0,0}};
...
volatile static AdminMultiReqType AdminMultiReq;
 ...
 */
boolean NvM_CheckMemIfJobFinished(void)
{
	MemIf_JobResultType jobResult;
...
static void NvM_ReportCrcError(void)
{
	if(nvmCrcErrorCnt < 0xFFFFFFFF)
	{
			nvmCrcErrorCnt++;
	}
	else
...
static void NvM_ReportWriteError(void)
{
	if(nvmWriteErrorCnt < 0xFFFFFFFF)
	{
			nvmWriteErrorCnt++;
	}
	else
							...
							boolean restoreFromRom )
{
		...
		if( MEMIF_JOB_PENDING == jobResult ) {
			...
			if( write ) {
				if( 0 == handleRedundantBlock(bPtr,admPtr) ) {
					if(bPtr->BlockManagementType == NVM_BLOCK_REDUNDANT )  /* switch data index */
					{
						admPtr->DataIndex = ((admPtr->DataIndex) ? 0 : 1); 
						admPtr->BlockState = BLOCK_STATE_MEMIF_REQ;
				...
				if( bPtr->BlockUseCrc ) {
					...
					if( bPtr->CalcRamBlockCrc ) {
						...
						/* savedDataPtr points to the real data buffers and they do no contain the crcLen */
						Irq_Save();
	...
	case BLOCK_STATE_CALC_CRC_WRITE:
	{
		...
		if( bPtr->BlockCRCType == NVM_CRC16 ) {
			...
			crc16 = Crc_CalculateCRC16(Nvm_WorkBuffer,bPtr->NvBlockLength,0xffff, TRUE);
			...
			/* @req 3.1.5/NVM253 */
			crc32 = Crc_CalculateCRC32(Nvm_WorkBuffer,bPtr->NvBlockLength,0xffffffffUL,TRUE);
	...
	case BLOCK_STATE_CALC_CRC_READ:
	{
		...
		case BLOCK_SUBSTATE_0:
			...
			if( checksumOk ) {
				...
				admPtr->ErrorStatus = NVM_REQ_OK;
				admPtr->BlockState = BLOCK_STATE_MEMIF_REQ;
				admPtr->DataIndex = 0;   /* if Read Block is Ok£¬So reset the block DataIndex and flags ,The next Req can Read/Write the Main Block not Redundant Block */
				admPtr->flags = 0;
				blockDone = 1;
				nvmBlockCrcVal[bPtr->NvramBlockIdentifier -1][admPtr->DataIndex] = 0;
			} else {
				/* NVM387, NVM388
				 ...
				 */
				 #if 0
				if( bPtr->BlockCRCType == NVM_CRC16 ) {
					nvmBlockCrcVal[bPtr->NvramBlockIdentifier -1][admPtr->DataIndex] = crc16;
				} else {
					nvmBlockCrcVal[bPtr->NvramBlockIdentifier -1][admPtr->DataIndex] = crc32;
				}
				#else
					nvmBlockCrcVal[bPtr->NvramBlockIdentifier -1][admPtr->DataIndex] = 1;
				#endif
				if( 0 == handleRedundantBlock(bPtr,admPtr) ) {
					/* report crc error*/  
					if(bPtr->BlockManagementType == NVM_BLOCK_REDUNDANT )
...
{
	DET_VALIDATE_NO_RV(nvmState != NVM_UNINITIALIZED, NVM_READ_ALL_ID, NVM_E_NOT_INITIALIZED);
	if(AdminMultiReq.state == NVM_WRITE_ALL)
	{
		if((nvmSubState == NS_PENDING) && (nvmSubState == NS_INIT))
		{
			/* if Write All Task is Pending ,Cancel it,And the state of NvM goto IDLE */
			Irq_Save();
			AdminMultiBlock.ErrorStatus = NVM_REQ_OK;
			nvmState = NVM_IDLE;
			nvmSubState = 0;
...
Std_ReturnType NvM_RestoreBlockDefaults( NvM_BlockIdType blockId, uint8* NvM_DestPtr )
{
	...
	qEntry.blockId = blockId;
	qEntry.dataPtr = (uint8 *)NvM_DestPtr;
...
Std_ReturnType NvM_ReadBlock( NvM_BlockIdType blockId, void* NvM_DstPtr )
{
    ...
    */
	...
	qEntry.blockId = blockId;
	qEntry.dataPtr = (uint8*)NvM_DstPtr;
...
Std_ReturnType NvM_WriteBlock( NvM_BlockIdType blockId, const void* NvM_SrcPtr ) {
	...
	qEntry.blockId = blockId;
	qEntry.dataPtr = (uint8 *)NvM_SrcPtr;
 ...
 *
 */
volatile NvmStateType currentNvMState;
...
//	static uint32 crc32Left;
	currentNvMState = nvmState;
	/* Check for new requested state changes */
	if( currentNvMState == NVM_IDLE ) 
	{
	    /* req 4.0.3|3.1.5/NVM273 */
...
boolean NvM_GetIsCrcInvalid(void)
{
...
boolean NvM_GetIsWriteErr(void)
{
	boolean result = FALSE;
	if(nvmWriteErrorCnt == 0)
	{
		result = FALSE;
	}
	else
...
void NvM_SetFirstUseFlag(boolean flg)
{
	nvmbFirstUsed = flg;
}


boolean NvM_GetFirstUseFlag(void)
{
	return (nvmbFirstUsed == TRUE);
}


boolean NvM_GetIsJobError(void)
{
	boolean result = FALSE;
	if(MemIfJobAdmin.JobErrorCnt == 0)
	{
		result = FALSE;
	}
	else
...
uint32 NvM_GetBlockCrcErrorVal(NvM_BlockIdType blockId, uint8 blockDataIndex)
{
	...
	return nvmBlockCrcVal[blockId-1][blockDataIndex];
}


boolean NvM_IsBlockNoError(NvM_BlockIdType blockId)
{
	if ((blockId == 0) ||(blockId > NVM_NUM_OF_NVRAM_BLOCKS))
	{
		return 0;
	}
	return AdminBlock[blockId-1].ErrorStatus == NVM_REQ_OK;
}
