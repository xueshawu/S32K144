...
#define EEP_AR_RELEASE_MINOR_VERSION_C       3
/*
...
*/
#define EEP_AR_RELEASE_REVISION_VERSION_C    1
...
static VAR(Eep_LengthType, EEP_VAR) Eep_Eeprom_u32RemainingLength = 0UL;
#endif
...
static FUNC( uint8, EEP_CODE) Eep_Eeprom_CalculateAlignment
(
    VAR( uint32, AUTOMATIC )  u32DestAddress,
    VAR( uint32, AUTOMATIC )  u32SrcAddress,
    VAR( Eep_LengthType, AUTOMATIC )  u32LocalTransferSize
);
/*==================================================================================================
                                       LOCAL FUNCTIONS
==================================================================================================*/
...
static FUNC( void, EEP_CODE ) Eep_Eeprom_ReportDetTimeout(void)
{
    ...
    switch (Eep_Eeprom_eJob)
    {
        ...
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, apiID, EEP_E_TIMEOUT );
}
#endif
#if ((EEP_CANCEL_API == STD_ON) || (EEP_ASYNC_OPERATIONS_ENABLED == STD_ON))
/**
...
static FUNC( Eep_Eeprom_ReturnType, EEP_CODE ) Eep_Eeprom_CheckFlexRamReady( void )
{
    ...
    MCAL_FAULT_INJECTION_POINT(EEP_FIP_01_EEP_CHECK_RAM_READY);
    /* get hardware flags */
...
static FUNC( void, EEP_CODE ) Eep_Eeprom_ReportBrownOut( VAR(uint8, AUTOMATIC) u8BrownOutCode)
{
    switch ((Eep_Eeprom_BrownOutCodeType)u8BrownOutCode)
    {
        case EEP_BO_DURING_MAINTENANCE_DETECTED:
            ...
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_BOMaintenanceCfg.id, DEM_EVENT_STATUS_FAILED);
            }
        break;
        case EEP_BO_DURING_QUICK_WRITES_DETECTED:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_BOQuickWritesCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_BOQuickWritesCfg.id, DEM_EVENT_STATUS_FAILED);
            }
        break;
        case EEP_BO_DURING_NORMAL_WRITES:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_BONormalWritesCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_BONormalWritesCfg.id, DEM_EVENT_STATUS_FAILED);
            }
        break;
...
)
{
    ...
    P2CONST(uint16, AUTOMATIC, EEP_APPL_CONST )  pu16TempSrc = NULL_PTR;
    P2VAR( uint16, AUTOMATIC, EEP_APPL_DATA )  pu16TempDest = NULL_PTR;
        ...
        switch(u8PageSize)
        {
            case EEP_PAGE_SIZE_1BYTE:
                ...
                *pu8TempDest = *pu8TempSrc;
                break;
            case EEP_PAGE_SIZE_2BYTES:
                if (EEP_JOB_WRITE == Eep_Eeprom_eJob)
                {
                    /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer */
                     /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
                    pu16TempSrc = (uint16 *) Eep_Eeprom_u32SrcAddress;
                }
                else
                {
                    /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer */
                    ...
                    pu16TempSrc = (uint16 *)Eep_Eeprom_u8ErasedValue;
                }
                /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer */
                 /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
                pu16TempDest = (uint16 *) Eep_Eeprom_u32DestAddress;
                *pu16TempDest = *pu16TempSrc;
                break;
...
)
{
    ...
    VAR( uint8, AUTOMATIC) u8PageSize = EEP_PAGE_SIZE_0BYTES;
    ...
    while (u32LocalTransferSize > 0UL)
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(u32TempDest, u32TempSrc, u32LocalTransferSize);
        ...
        switch (u8PageSize)
        {
            ...
            case EEP_PAGE_SIZE_2BYTES:
                /* @violates @ref eep_c_REF_25 Conversions shall not be  performed between a pointer */
                /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
                if ( *((uint16 *)u32TempDest) != *((uint16 *)u32TempSrc))
                {
                    SchM_Exit_Eep_EEP_EXCLUSIVE_AREA_09();
                    /* @violates @ref eep_c_REF_5 Return statement before end of function. */
                    return EEPROM_E_BLOCK_INCONSISTENT;
        ...
        u32TempSrc = u32TempSrc + u8PageSize;
        u32TempDest = u32TempDest + u8PageSize;
        u32LocalTransferSize = u32LocalTransferSize - u8PageSize;
    }
    ...
    while (u32LocalTransferSize > 0UL)
    {
        ...
        switch (u8PageSize)
        {
            ...
            case EEP_PAGE_SIZE_2BYTES:
                /* @violates @ref eep_c_REF_25 Conversions shall not be  performed between a pointer */
                /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
                *((uint16 *)u32TempDest) = *((uint16 *)u32TempSrc);
                break;
            case EEP_PAGE_SIZE_1BYTE:
                /* @violates @ref eep_c_REF_25 Conversions shall not be  performed between a pointer */
                /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
...
)
{
    ...
    while (u32LocalTransferSize > 0UL)
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(Eep_Eeprom_u32DestAddress, Eep_Eeprom_u32SrcAddress, u32LocalTransferSize);

        eRetVal = Eep_Eeprom_WritePageSync(u8PageSize);
        u32LocalTransferSize = u32LocalTransferSize - u8PageSize;
...
static FUNC( void, EEP_CODE ) Eep_Eeprom_WriteAsync(void)
{
    ...
    {
        /* A request to write the last page was sent to the controller. Just
        ...
        {
            /* If it has not failed until now, the write was successful */
    ...
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(Eep_Eeprom_u32DestAddress, Eep_Eeprom_u32SrcAddress, Eep_Eeprom_u32RemainingLength);

        eRetVal = Eep_Eeprom_WaitIdleOrWritePageAsync(u8PageSize);
    }
}
...
/**
* @brief Calculates the alignment and returns the maximum size available for the Read/Write operation.
* @return uint8 value in range: EEP_PAGE_SIZE_1BYTE EEP_PAGE_SIZE_2BYTES EEP_PAGE_SIZE_4BYTES
*
*/
static FUNC( uint8, EEP_CODE) Eep_Eeprom_CalculateAlignment
(
...
{
    VAR( uint8, AUTOMATIC) u8PageSize = EEP_PAGE_SIZE_1BYTE;

    /* 4 bytes aligned operation */
    if( (0U == (u32DestAddress % EEP_PAGE_SIZE_4BYTES)) &&
        (0U == (u32SrcAddress  % EEP_PAGE_SIZE_4BYTES))   )
    {
        /* 4 bytes aligned and length >= 4 bytes: 4 bytes operation */
        if(u32LocalTransferSize >= EEP_PAGE_SIZE_4BYTES)
        {
            u8PageSize = EEP_PAGE_SIZE_4BYTES;
        }
        /* 4 bytes aligned and 4bytes > length >= 2 bytes : 2 bytes operation */
        else if(u32LocalTransferSize >= EEP_PAGE_SIZE_2BYTES)
        {
            u8PageSize = EEP_PAGE_SIZE_2BYTES;
        }
        /* 4 bytes aligned and length < 2 bytes : 1 byte operation */
        else
        {
            u8PageSize = EEP_PAGE_SIZE_1BYTE;
        }
    }
    /* 2 bytes aligned operation */
    else if( (0U == (u32DestAddress % EEP_PAGE_SIZE_2BYTES)) &&
             (0U == (u32SrcAddress  % EEP_PAGE_SIZE_2BYTES))   )
    {
        /* 2 bytes aligned and length >= 2 bytes : 2 bytes operation */
        if(u32LocalTransferSize >= EEP_PAGE_SIZE_2BYTES)
        {
            u8PageSize = EEP_PAGE_SIZE_2BYTES;
        }
...
( P2CONST( Eep_ConfigType, AUTOMATIC, EEP_APPL_CONST ) pConfigPtr )
{
    ...
    {
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_INIT_ID, EEP_E_BUSY );
    }
    else
    {
#endif
...
)
{
    ...
    else
    {
        ...
        {
        #if( EEP_DEV_ERROR_DETECT == STD_ON )
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_ERASE_ID, EEP_E_BUSY );
        #endif
            u8RetVal = (Std_ReturnType)E_NOT_OK;
...
)
{
    ...
    else
    {
        ...
        {
#if( EEP_DEV_ERROR_DETECT == STD_ON )
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_WRITE_ID, EEP_E_BUSY );
#endif
            u8RetVal = (Std_ReturnType)E_NOT_OK;
...
)
{
    ...
    else
    {
        ...
        {
#if( EEP_DEV_ERROR_DETECT == STD_ON )
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_READ_ID, EEP_E_BUSY );
#endif    /* EEP_DEV_ERROR_DETECT == STD_ON */
            u8RetVal = (Std_ReturnType)E_NOT_OK;
        }
        else
...
)
{
    ...
    else
    {
        ...
        {
#if( EEP_DEV_ERROR_DETECT == STD_ON )
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_COMPARE_ID, EEP_E_BUSY );
#endif    /* EEP_DEV_ERROR_DETECT == STD_ON */
            u8RetVal = (Std_ReturnType)E_NOT_OK;
        }
        else
...
)
{
    ...
    {
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_SETMODE_ID, EEP_E_BUSY );
    }
    else
    {
#endif  /* EEP_DEV_ERROR_DETECT == STD_ON */
...
)
{
    ...
    else
    {
        ...
        {
            #if( EEP_DEV_ERROR_DETECT == STD_ON )
        (void) Det_ReportRuntimeError( (uint16)EEP_MODULE_ID, EEP_INSTANCE_ID, EEP_QUICK_WRITE_ID, EEP_E_BUSY );
            #endif
            u8RetVal = (Std_ReturnType)E_NOT_OK;
...
static FUNC( void, EEP_CODE ) Eep_ReportDemErrors(void)
{
    switch( Eep_eJob )
    {
        case EEP_JOB_ERASE:
            ...
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_EraseFailedCfg.id, DEM_EVENT_STATUS_FAILED);
            }
            break;
        case EEP_JOB_WRITE:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_WriteFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_WriteFailedCfg.id, DEM_EVENT_STATUS_FAILED);
            }
            break;
        case EEP_JOB_READ:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_ReadFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_ReadFailedCfg.id, DEM_EVENT_STATUS_FAILED);
            }
            break;
        #if(EEP_COMPARE_API == STD_ON)
        case EEP_JOB_COMPARE:
            ...
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_CompareFailedCfg.id, DEM_EVENT_STATUS_FAILED);
            }
            break;
        #endif
        default:
...
static FUNC( void, EEP_CODE ) Eep_ReportDemPasses(void)
{
    switch( Eep_eJob )
    {
        case EEP_JOB_ERASE:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_EraseFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_EraseFailedCfg.id, DEM_EVENT_STATUS_PASSED);
            }
            break;
        case EEP_JOB_WRITE:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_WriteFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_WriteFailedCfg.id, DEM_EVENT_STATUS_PASSED);
            }
            break;
        case EEP_JOB_READ:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_ReadFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_ReadFailedCfg.id, DEM_EVENT_STATUS_PASSED);
            }
            break;
        #if(EEP_COMPARE_API == STD_ON)
        case EEP_JOB_COMPARE:
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_CompareFailedCfg.state)
            {
                (void)Dem_SetEventStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_CompareFailedCfg.id, DEM_EVENT_STATUS_PASSED);
            }
            break;
        #endif
        default:
    ...
    switch( eJob )
    {
        case EEP_JOB_ERASE:
                /*for FLEXRAM ERASE will be implemented by writing the erased value
                  because there is no FLEXRAM support for erase */
