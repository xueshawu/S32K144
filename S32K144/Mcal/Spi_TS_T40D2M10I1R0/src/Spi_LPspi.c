...
*/
#define SPI_AR_RELEASE_MINOR_VERSION_LPSPI_C         3
/*
...
*/
#define SPI_AR_RELEASE_REVISION_VERSION_LPSPI_C      1
...
#if (SPI_VENDOR_ID_LPSPI_C != REG_ESYS_LPSPI_VENDOR_ID_REGFLEX)
...
#if ((SPI_AR_RELEASE_MAJOR_VERSION_LPSPI_C    != REG_ESYS_LPSPI_AR_RELEASE_MAJOR_VERSION_REGFLEX) || \
     (SPI_AR_RELEASE_MINOR_VERSION_LPSPI_C    != REG_ESYS_LPSPI_AR_RELEASE_MINOR_VERSION_REGFLEX) || \
     (SPI_AR_RELEASE_REVISION_VERSION_LPSPI_C != REG_ESYS_LPSPI_AR_RELEASE_REVISION_VERSION_REGFLEX))
#error "AutoSar Version Numbers of Spi_LPspi.c and Reg_eSys_LPSPI.h are different"
#endif
#if ((SPI_SW_MAJOR_VERSION_LPSPI_C  != REG_ESYS_LPSPI_SW_MAJOR_VERSION_REGFLEX) || \
     (SPI_SW_MINOR_VERSION_LPSPI_C  != REG_ESYS_LPSPI_SW_MINOR_VERSION_REGFLEX) || \
     (SPI_SW_PATCH_VERSION_LPSPI_C  != REG_ESYS_LPSPI_SW_PATCH_VERSION_REGFLEX))
...
*                                       LOCAL MACROS
==================================================================================================*/
#define LPSPI_DMA_MAX_LINKED_ITER_CNT  (0x7FFFu)
...
{
    P2CONST(Spi_Ipw_LPspiDeviceAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) LPspi_pcDeviceAttributesConfig;
    P2VAR(Spi_LPspi_DeviceStateType, AUTOMATIC, SPI_APPL_DATA) LPspi_pDev;
    ...
    for (LPspi_u32Job = 0u; LPspi_u32Job <= (uint32)(Spi_pcSpiConfigPtr->Spi_Max_Job); LPspi_u32Job++)
    {
        ...
        */        
        ...
        if (LPspi_pcJobConfig->HWUnit == HWUnit)
        {
            ...
            * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
            */
            LPspi_pcDeviceAttributesConfig = (Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiDeviceAttributesConfig)[LPspi_pcJobConfig->ExternalDevice];
...
#if ( (SPI_LEVEL_DELIVERED == LEVEL1) || (SPI_LEVEL_DELIVERED == LEVEL2) )
#if (SPI_DMA_USED == STD_ON)
    if (1u == LPspi_pcHWUnitConfig->u16UseDma)
    {
    ...
    if (SPI_PHYUNIT_ASYNC_U32 == LPspi_pcHWUnitConfig->u32IsSync)
    {
        ...
        if (SPI_INTERRUPT_MODE == AsyncMode)
        {
            ...
            else
            {
                ...
                if (SPI_SLAVE == LPspi_pcHWUnitConfig->eSpiPhyUnitMode)
                {
                    /*
                    * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
                    *     type and an integral type.
        ...
        if (0u != LPspi_u32IsrStatus)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    LPspi_u32Cmd &= ~(LPSPI_TCR_CONT_EN_U32 | LPSPI_TCR_CONTC_EN_U32);
                    /*
                    * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
                    *     type and an integral type.
                    */
...
{
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) LPspi_pcChannelAttributesConfig;
    P2CONST(Spi_ChannelConfigType, AUTOMATIC, SPI_APPL_CONST) LPspi_pcChannelConfig;
    P2CONST(Spi_ChannelStateType, AUTOMATIC, SPI_APPL_CONST) LPspi_pChannelState;
    ...
    * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
    */
    LPspi_pcChannelAttributesConfig = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[Channel];
    ...
    if (0u != (LPspi_pChannelState->u8Flags & ((uint8)SPI_CHANNEL_FLAG_TX_DEFAULT_U8)))
    {
        ...
        LPspi_pDev->LPspi_u32TransmitDefaultData = LPspi_pcChannelConfig->u32DefaultTransmitValue;
    }
    else
    {
    ...
    */
    if(0u!=(LPspi_u32Cmd & LPSPI_TCR_CONT_EN_U32 & LPSPI_TCR_RESERVED_MASK_U32))
    {
        if((uint8)TRUE == LPspi_pDev->LPspi_FirstChannelOfJobFlag)
        {
            ...
            * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
            */
            REG_WRITE32(LPspi_pDev->LPspi_u32TCRAddress, (LPSPI_TCR_CONTC_EN_U32 & LPSPI_TCR_RESERVED_MASK_U32));
        ...
        else
        {
            ...
            * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
            */
            REG_WRITE32(LPspi_pDev->LPspi_u32TCRAddress, (LPspi_u32Cmd | LPSPI_TCR_CONTC_EN_U32) & LPSPI_TCR_RESERVED_MASK_U32);
        }
...
#if (SPI_SLAVE_SUPPORT == STD_ON)
    if(SPI_SLAVE == LPspi_pDev->LPspi_pcHWUnitConfig->eSpiPhyUnitMode)
    {
        /*
         * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
         *     type and an integral type.
    ...
    ) \
{
    ...
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) 
        /*
        * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
        */
        LPspi_pcChannelAttributesConfig = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[*(LPspi_pDev->LPspi_pcCurrentChannelIndexPointerTX)];
...
#endif
    if(SPI_MASTER == LPspi_pDev->LPspi_pcHWUnitConfig->eSpiPhyUnitMode)
    {
    ...
    if(SPI_SLAVE == LPspi_pDev->LPspi_pcHWUnitConfig->eSpiPhyUnitMode)
    {
        if(LPspi_nFramesCount > SPI_FIFO_SIZE_LPSPI)
        {
            /*
             * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
             *     type and an integral type.
            ...
            * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
            */
            LPspi_nFramesCount = (Spi_NumberOfDataType)((uint32)SPI_FIFO_SIZE_LPSPI - LPSPI_FSR_TXCOUNT_VAL_U32(REG_READ32(LPSPI_FSR_ADDR32(LPspi_pDev->LPspi_pcHWUnitConfig->u8Offset))));
        }
        else
        {
            /*
            ...
            * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
            */
            if(LPspi_nFramesCount > (Spi_NumberOfDataType)((uint32)SPI_FIFO_SIZE_LPSPI - LPSPI_FSR_TXCOUNT_VAL_U32(REG_READ32(LPSPI_FSR_ADDR32(LPspi_pDev->LPspi_pcHWUnitConfig->u8Offset)))))
            {
               /*
                * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
                *     type and an integral type.
...
{
     P2CONST(Spi_DataBufferType, AUTOMATIC, SPI_APPL_CONST) LPspi_pBufferTX = LPspi_pDev->LPspi_pCurrentTxBuffer;
    ...
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) 
        ...
        */
        LPspi_pcChannelAttributesConfig = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[*(LPspi_pDev->LPspi_pcCurrentChannelIndexPointerTX)];
#if (SPI_FORCE_DATA_TYPE == STD_ON)
    P2CONST(uint16, AUTOMATIC, SPI_APPL_CONST) LPspi_pu16BufferFullTX;
    P2CONST(uint32, AUTOMATIC, SPI_APPL_CONST) LPspi_pu32BufferFullTX;
#else
        ...
        */
        ...
        while (LPspi_nFramesCount > 0u)
        {
            ...
            if( SPI_DATA_WIDTH_16 == LPspi_pcChannelAttributesConfig->u8DataWidth)
            {
                ...
                */
                /*
                * @violates @ref Spi_LPspi_c_REF_13 A cast shall not be performed that removes any const or volatile
                * qualification from the type addressed by a pointer.
                */
    ...
    ) \
{
    ...
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) \
        ...
        * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
        */
        LPspi_pcChannelAttributesConfig = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[*(LPspi_pDev->LPspi_pcCurrentChannelIndexPointerRX)];
    #if (SPI_FORCE_DATA_TYPE == STD_ON)
    ...
    for (LPspi_u32Sequence = 0u; LPspi_u32Sequence <= (uint32)(Spi_pcSpiConfigPtr->Spi_Max_Sequence); LPspi_u32Sequence++)
    {
        ...
        */
        ...
        if (1u == LPspi_pcSequenceConfig->NumJobs)
        {
            ...
            * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
            */
            u8HWtypeOptimized = Spi_pcSpiConfigPtr->pcHWUnitConfig[LPspi_pcJobConfig->HWUnit].u8HWtype;
            ...
            if (LPspi_pcJobConfig->NumChannels <= (SPI_OPTIMIZED_CHANNEL_BUFFER_SIZE-LPspi_ChannelsAttrCurrentIndex))
            {
                ...
                * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
                */
                LPspi_pcDeviceAttributesConfig = (Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiDeviceAttributesConfig)[Spi_pcSpiConfigPtr->pcJobConfig[LPspi_Job].ExternalDevice];

                /* prepare sequence cache information */
                LPspi_pSeqAttrs->u8HWtypeOptimized = u8HWtypeOptimized;
                ...
                for (LPspi_Channel = 0u; LPspi_Channel < LPspi_pcJobConfig->NumChannels; LPspi_Channel++)
                {
                    ...
                    */
                    ...
                    LPspi_pChannelAttrs->LPspi_u32DefaultTransmitValue = LPspi_pcChannelConfig->u32DefaultTransmitValue;
                    ...
                    LPspi_pChannelAttrs->LPspi_u32TCR[0] =
                        ...
                        * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
                        */
                        Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[LPspi_ChannelId].u32CMD;
                    if ((uint8)TRUE == LPspi_pcDeviceAttributesConfig->u8DualClk)
                    {    
                    ...
                    LPspi_pChannelAttrs->LPspi_u8DataWidth =
                        ...
                        * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
                        */
                        Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[LPspi_ChannelId].u8DataWidth;
                } /* for (channels) */
...
#if (SPI_DUAL_CLOCK_MODE == STD_ON)
    if((uint8)TRUE == (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u8DualClk)
    {
        LPspi_pDev->LPspi_u32ExternalDeviceTCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR[Spi_LPspi_ClockMode];
        LPspi_pDev->LPspi_u32ExternalDeviceCCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR[Spi_LPspi_ClockMode];
    }
    else
    {
        LPspi_pDev->LPspi_u32ExternalDeviceTCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR[SPI_NORMAL];
        LPspi_pDev->LPspi_u32ExternalDeviceCCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR[SPI_NORMAL]; 
    } 
#else
    LPspi_pDev->LPspi_u32ExternalDeviceTCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR;
    LPspi_pDev->LPspi_u32ExternalDeviceCCR = (*pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR;
#endif
...
)
{
    ...
    if(0u != LPspi_u32IsrStatusTX)
    {
        ...
        else
        {
            ...
            LPspi_u32Cmd = LPspi_u32Cmd & (~(LPSPI_TCR_CONT_EN_U32 | LPSPI_TCR_CONTC_EN_U32)); /* clear the continuous CS assertion */
            /*
            * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
            *     type and an integral type.
            */
...
#if (SPI_DUAL_CLOCK_MODE == STD_ON)
        if((uint8)TRUE == (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u8DualClk)
        {
            LPspi_u32JobCmd = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR[Spi_LPspi_ClockMode];
            LPspi_u32CCR = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR[Spi_LPspi_ClockMode];
        }
        else
        {
            LPspi_u32JobCmd = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR[SPI_NORMAL];
            LPspi_u32CCR = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR[SPI_NORMAL]; 
        }        
#else
        LPspi_u32JobCmd = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32TCR;
        LPspi_u32CCR = (*LPspi_pcJobConfig->ExternalDeviceAttrs->pcLPspiExternalDeviceAttrs)->u32CCR;
#endif 
        /*
        * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
        *     type and an integral type.
...
* file scope shall have internal linkage unless external linkage is required.
*/
FUNC(Std_ReturnType, SPI_CODE) Spi_LPspi_SyncTransmitJob
(
    P2VAR(Spi_LPspi_SyncStateType, AUTOMATIC,SPI_APPL_DATA) Spi_LPspi_pSyncTransmitState
)
{
    ...
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) LPspi_pcChannelAttributesCfgTX = NULL_PTR;
    P2CONST(Spi_Ipw_LPspiChannelAttributesConfigType, AUTOMATIC, SPI_APPL_CONST) LPspi_pcChannelAttributesCfgRX = NULL_PTR;
    ...
    VAR(sint32, AUTOMATIC) LPspi_s32Timeout = SPI_TIMEOUT_COUNTER; 
    ...
    VAR(uint32, AUTOMATIC) LPspi_u32FSR_RX_TX_Status = 0u;
    ...
    while((Spi_ChannelType)0 < Spi_LPspi_pSyncTransmitState->LPspi_ChannelsCountTX)
    {
        ...
        * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
        */
        LPspi_pcChannelAttributesCfgTX = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[LPspi_ChannelTX];
        ...
        */
        ...
        Spi_LPspi_pSyncTransmitState->LPspi_u32DefaultTransmitValue = LPspi_pcChannelConfigTX->u32DefaultTransmitValue;
        ...
        */
        if(0u != (Spi_LPspi_pSyncTransmitState->LPspi_u32Cmd & LPSPI_TCR_CONT_EN_U32 & LPSPI_TCR_RESERVED_MASK_U32))
        {
            if((uint8)TRUE == LPspi_FirstChannelOfJobFlag)
            {
                ...
                * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
                */
                REG_WRITE32(LPSPI_BASEADDR2TCR_ADDR32(LPspi_u32BaseAddress), (LPSPI_TCR_CONTC_EN_U32 & LPSPI_TCR_RESERVED_MASK_U32));
            }
            else
            {
                /*
                ...
                * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
                */
                REG_WRITE32(LPSPI_BASEADDR2TCR_ADDR32(LPspi_u32BaseAddress), ((Spi_LPspi_pSyncTransmitState->LPspi_u32Cmd | LPSPI_TCR_CONTC_EN_U32) & LPSPI_TCR_RESERVED_MASK_U32));
            }
        }
        else
        {
        ...
        if (1u == (LPspi_pChannelState->u8Flags & ((uint8)SPI_CHANNEL_FLAG_TX_DEFAULT_U8)))
        {
        ...
        if((uint8)TRUE == LPspi_FirstChannelOfJobFlag)
        {
            ...
            * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
            */
            LPspi_pcChannelAttributesCfgRX = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[LPspi_ChannelRX];
            /*
            * @violates @ref Spi_LPspi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
            */
        ...
        LPspi_s32Timeout = SPI_TIMEOUT_COUNTER;
        ...
        * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
        */
        LPspi_u32FSR_RX_TX_Status = REG_READ32(LPSPI_BASEADDR2FSR_ADDR32(LPspi_u32BaseAddress)) & (LPSPI_FSR_RXCOUNT_MASK_U32 | LPSPI_FSR_TXCOUNT_MASK_U32);
        while(
                ((Spi_LPspi_pSyncTransmitState->LPspi_LengthTX>0u) && (Spi_LPspi_pSyncTransmitState->LPspi_EndOfJobFlag==(uint8)FALSE)) ||
                ((Spi_LPspi_pSyncTransmitState->LPspi_EndOfJobFlag==(uint8)TRUE) && ((Spi_LPspi_pSyncTransmitState->LPspi_LengthTX>0u) || (Spi_LPspi_pSyncTransmitState->LPspi_LengthRX>0u))) || (LPspi_u32FSR_RX_TX_Status != 0u)
             )
        {
            /*
            * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
...
{
    P2CONST(Spi_LPspi_ChannelCacheAttrsType, AUTOMATIC, SPI_APPL_CONST) LPspi_pChannelTX;    
    ...
    *(pcSequence->LPspi_pJobStatus) = SPI_BUSY;
    ...
    {
        Spi_LPspi_pSyncTransmitState->LPspi_ChannelsCountTX--;
        Spi_LPspi_pSyncTransmitState->LPspi_u32DefaultTransmitValue = LPspi_pChannelTX->LPspi_u32DefaultTransmitValue;
        ...
        */
        ...
        /* retrieve TX buffers and the transmitted length */
        if (1u == (LPspi_pChannelTX->LPspi_pState->u8Flags & ((uint8)SPI_CHANNEL_FLAG_TX_DEFAULT_U8)))
        {
            Spi_LPspi_pSyncTransmitState->LPspi_pBufferTX = NULL_PTR;
        }
        else
...
)
{
    ...
    */
    ...
    if (
         ...
         ((LPspi_u32FSR & LPSPI_FSR_TXCOUNT_MASK_U32)<SPI_FIFO_SIZE_LPSPI)
       )
    {
        ...
        if (NULL_PTR == LPspi_pBufferTX)
        {
            ...
            LPspi_u32TempXferData = Spi_LPspi_pSyncTransmitState->LPspi_u32DefaultTransmitValue;
        }
        else
        {
            /* Read transmit data */
...
)
{
    ...
    if (0u < Spi_LPspi_pSyncTransmitState->LPspi_LengthRX)
    {
        ...
        */
        ...
        {
            Spi_LPspi_pSyncTransmitState->LPspi_u32Cmd &= ~(LPSPI_TCR_CONT_EN_U32 | LPSPI_TCR_CONTC_EN_U32);
    ...
    ) \
{
    ...
    */
    ...
    P2CONST(Spi_ChannelStateType, AUTOMATIC, SPI_APPL_CONST) LPspi_pChannelState = LPspi_pcChannelConfig->pChannelState;
    ...
    */
    LPspi_pcChannelAttributesConfig = &Spi_pcSpiConfigPtr->pcAttributesConfig->pcLPspiChannelAttributesConfig[Channel];
    /* Get the current command for this channel */
    LPspi_u32Cmd = (LPspi_pDev->LPspi_u32ExternalDeviceTCR | LPspi_pcChannelAttributesConfig->u32CMD);
    /* Store the current command for this channel */
    LPspi_pDev->LPspi_u32CurrentCmd = LPspi_u32Cmd;
    /* Get the configuration of FIFO for this channel */
    LPspi_u32FCR = (LPspi_pcChannelAttributesConfig->u32FCR);
    ...
    */
    ...
    if (0u != (LPspi_pChannelState->u8Flags & ((uint8)SPI_CHANNEL_FLAG_TX_DEFAULT_U8)))
    {
        ...
        * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
        */
        Mcl_DmaTcdSetSaddr(LPspi_pTxTcdAddr, (uint32)(&LPspi_pcChannelConfig->u32DefaultTransmitValue));
    ...
    else
    {
        ...
        */
        ...
        else if (SPI_DATA_WIDTH_32 == LPspi_pcChannelAttributesConfig->u8DataWidth)
        {
            ...
            /* 4 bytes src offset */
            Mcl_DmaTcdSetSoff(LPspi_pTxTcdAddr, (sint16)4u);
            Mcl_DmaTcdSetSModuloAndSize(LPspi_pTxTcdAddr, (uint8)0x00u, DMA_SIZE_4BYTES);
    ...
    * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
    */
    REG_BIT_CLEAR32(LPSPI_DER_ADDR32(LPspi_u8HWoffset), (uint32)(LPSPI_DER_TDDE_MASK_U32));
    /* TX HW request */
    Mcl_DmaEnableHwRequest(LPspi_pcHWUnitConfig->u8TxDmaChannel);
    /* Transfer the first frame of TX */
    Mcl_DmaStartChannel(LPspi_pcHWUnitConfig->u8TxDmaChannel);
    ...
    ) \
{
    ...
    else
    {
        ...
        */
        ...
        else if (SPI_DATA_WIDTH_32 == LPspi_pcChannelAttributesConfig->u8DataWidth)
        {
            ...
            */
            Mcl_DmaTcdSetIterCount(LPspi_pRxTcdAddr, (uint16)(LPspi_FramesCount));
            Mcl_DmaTcdSetDoff(LPspi_pRxTcdAddr, (sint16)4u);
            /* Compiler_Warning: It is intended for the address of the element to be passed
                                  to the function as an unsigned integer*/
            Mcl_DmaTcdSetSaddr(LPspi_pRxTcdAddr, LPspi_pDev->LPspi_u32RDRAddress);
            /* overwrite the default src next offset */
            ...
            */
            ...
            Mcl_DmaTcdSetDModuloAndSize(LPspi_pRxTcdAddr, (uint8)0x00u, DMA_SIZE_4BYTES);
            /* 4 bytes to transfer */
            /*
            * @violates @ref Spi_LPspi_c_REF_3 A cast should not be performed between a pointer
            *     type and an integral type.
    ...
    * @violates @ref Spi_LPspi_c_REF_12 A cast should not be performed between a pointer type and an integral type.
    */ 
    REG_BIT_SET32(LPSPI_DER_ADDR32(LPspi_u8HWoffset), (uint32)(LPSPI_DER_TDDE_MASK_U32));
}
#endif /* SPI_DMA_USED == STD_ON */
/*================================================================================================*/
/**
...
FUNC(void, SPI_CODE) Spi_LPspi_SlaveCancel(VAR(Spi_SequenceType, AUTOMATIC) Sequence)
{
    ...
    VAR(Spi_JobType, AUTOMATIC) LPspi_Job;
    P2CONST(Spi_LPspi_DeviceStateType, AUTOMATIC, SPI_APPL_CONST) LPspi_pDev;
    P2CONST(Spi_SequenceStateType, AUTOMATIC, SPI_APPL_CONST) LPspi_pSequenceState;
