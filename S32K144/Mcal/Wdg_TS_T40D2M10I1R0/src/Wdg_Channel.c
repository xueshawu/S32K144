...
/** @violates @ref Wdg_c_REF_3 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define WDG_CHANNEL_AR_RELEASE_MINOR_VERSION_C       3
/** @violates @ref Wdg_c_REF_3 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define WDG_CHANNEL_AR_RELEASE_REVISION_VERSION_C    1
...
*/
static VAR(uint32, WDG_VAR) Wdg_au32Timeout[WDG_NO_OF_INSTANCES] = {(uint32)0x0};

/**
*  @brief Variable that indicated the timer value in ticks
*/
static VAR(uint32, WDG_VAR) Wdg_au32GptPeriod[WDG_NO_OF_INSTANCES] = {(uint32)0x0};
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        (void)pConfigPtr;
            ...
            if ((Std_ReturnType)E_OK == valid)
            {
                ...
                u32TimerValue = (u32TimerValue/(Wdg_apConfigPtr[Wdg_Instance]->Wdg_ModeSettings[Wdg_apConfigPtr[Wdg_Instance]->Wdg_DefaultMode]->Wdg_u32InternalTimerClock));
                u32TimerValue = u32TimerValue*(Wdg_apConfigPtr[Wdg_Instance]->Wdg_u32TriggerSourceClock);
                Wdg_au32GptPeriod[Wdg_Instance] = u32TimerValue;
                ...
                {
                    Gpt_StartTimer(Wdg_apConfigPtr[Wdg_Instance]->Wdg_TimerChannel,u32TimerValue);
                }
                                                 ...
                                                 )
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
            ...
            #endif
            #endif
                u32TimerValue = Wdg_IPW_SetMode(Wdg_apConfigPtr[Wdg_Instance]->Wdg_ModeSettings[Mode], Wdg_Instance);
                u32TimerValue = (u32TimerValue/(Wdg_apConfigPtr[Wdg_Instance]->Wdg_ModeSettings[Mode]->Wdg_u32InternalTimerClock));
                u32TimerValue = u32TimerValue*(Wdg_apConfigPtr[Wdg_Instance]->Wdg_u32TriggerSourceClock);
                Gpt_StopTimer(Wdg_apConfigPtr[Wdg_Instance]->Wdg_TimerChannel);
