...
* external identifiers.
*/
#define LIN_LPUART_AR_RELEASE_MINOR_VERSION_C       3
/**
* @violates @ref Lin_LPUART_c_REF_6 The compiler/linker shall be checked to
* ensure that 31 character significance and case sensitivity are supported for
* external identifiers.
*/
#define LIN_LPUART_AR_RELEASE_REVISION_VERSION_C    1
...
*/
 ...
 /**
* @brief        Store LPUART Checksum type
*/
static volatile VAR(boolean, LIN_VAR) bLPUART_ChecksumType;
...
*/
static  volatile VAR(uint8, LIN_VAR) Lin_Lpuart_au8Buffer[LIN_LPUART_MAX_MODULES][LPUART_MAX_BUFFER_LENGTH_U8];

/**
* @brief        Point to the next byte to send or receive in LPUART Buffers
*/
static volatile VAR(uint8, LIN_VAR) Lin_Lpuart_au8BufferPtr[LIN_LPUART_MAX_MODULES];

/**
* @brief        Real sizes of LPUART Buffers
*/
static volatile VAR(uint8, LIN_VAR) Lin_Lpuart_au8BufferLength[LIN_LPUART_MAX_MODULES];

static volatile VAR(uint8, LIN_VAR) Lin_Lpuart_au8WakeupFlag[LIN_LPUART_MAX_MODULES];
...
static FUNC (void, LIN_CODE) Lin_LPUART_CopyData(CONST(uint8, AUTOMATIC) u8Channel, P2VAR(uint8, AUTOMATIC, LIN_APPL_DATA) pu8LinSduPtr)
{
    ...
    for(u8Index = (uint8)0U; u8Index < u8BufferLength; u8Index++)
    {
        ...
        */
        pu8LinSduPtr[u8Index] = Lin_Lpuart_au8Buffer[u8Channel][LPUART_DATA_OFFSET_U8 + u8Index];
    }
}
...
static FUNC (void, LIN_CODE) Lin_LPUART_PIDCalc(VAR(uint8, AUTOMATIC) u8Channel, VAR(uint8, AUTOMATIC) u8FrameID)
{
    ...
    Lin_Lpuart_au8Buffer[u8Channel][LPUART_PID_BYTE_OFFSET_U8] = (uint8)u32Temp1;
}
...
static FUNC (uint32, LIN_CODE) Lin_LPUART_ChecksumCalc(VAR(uint8, AUTOMATIC) u8Channel, VAR(Lin_FrameCsModelType, AUTOMATIC) checksumModel)
{
    ...
    {
        u32Checksum = (uint32)Lin_Lpuart_au8Buffer[u8Channel][LPUART_PID_BYTE_OFFSET_U8];
    }

    for(u32Index = (uint32)0U; u32Index < u32DataLen; u32Index++)
    {
        u32Checksum += (uint32)Lin_Lpuart_au8Buffer[u8Channel][LPUART_DATA_OFFSET_U8 + u32Index];
...
static FUNC (void, LIN_CODE) Lin_LPUART_RxBuffFullInterruptHandler(CONST(uint8, AUTOMATIC) u8LPUART_Channel, VAR(uint8, AUTOMATIC) u8LogicChannel)
{
    ...
    VAR(uint8, AUTOMATIC) u8Lin_Checksum;
    ...
    */
    ...
    u8Temp1 = Lin_Lpuart_au8Buffer[u8LogicalChannel][u8BufferPtr];
    ...
    else
    {
        switch(u8BufferPtr)
        {
            ...
            case LPUART_SYNCH_BYTE_OFFSET_U8:
                if((uint8)(u32Lin_DataReg & LPUART_DATA_8BITS_MASK_U32) != u8Temp1)
                {
                    /* Bit error detected */
            ...
            default:
                switch(au8Lin_Command)
                {
                    ...
                    case LIN_TX_SLAVE_RES_COMMAND:
                        ...
                        else if(((uint8)(u32Lin_DataReg & LPUART_DATA_8BITS_MASK_U32) != u8Temp1) && (LPUART_PID_BYTE_OFFSET_U8 == u8BufferPtr))
                        {
                            /* Bit error detected */
                            Lin_au8LinChFrameErrorStatus[u8LogicalChannel] = LIN_BIT_ERROR;
                        }
                        else if(LPUART_PID_BYTE_OFFSET_U8 < u8BufferPtr)
                        {
                            /* Store received data */
                            Lin_Lpuart_au8Buffer[u8LogicalChannel][u8BufferPtr] = (uint8)(u32Lin_DataReg & LPUART_DATA_8BITS_MASK_U32);
                            ...
                            if(u8BufferPtr == (u8BufferLength - (uint8)1U))
                            {
                                u8Lin_FrameID = Lin_Lpuart_au8Buffer[u8LogicalChannel][LPUART_PID_BYTE_OFFSET_U8];
                                /* Checksum Error */
                                switch(u8Lin_FrameID)
                                {
                                    case LPUART_MASTER_REQUEST_DIAGNOSTIC_PID_U8:
                                    case LPUART_SLAVE_RESPONSE_DIAGNOSTIC_PID_U8:
                                        u8Lin_Checksum = (uint8)Lin_LPUART_ChecksumCalc(u8LogicalChannel, LIN_CLASSIC_CS);
                                    break;
                                    default:
                                        if ((boolean)TRUE == bLPUART_ChecksumType)
                                        {
                                            /* Enhance checksum */
                                            u8Lin_Checksum = (uint8)Lin_LPUART_ChecksumCalc(u8LogicalChannel, LIN_ENHANCED_CS);
                                        }
                                        else
                                        {
                                            /* Classic checksum */
                                ...
                                if(u8Lin_Checksum != (uint8)(u32Lin_DataReg & LPUART_DATA_8BITS_MASK_U32))
                                {
...
FUNC(void, LIN_CODE) Lin_LPUART_InitChannel(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    /* Check whether wake up detection is enabled */
     if ((uint8) STD_ON == Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->u8LinChannelWakeupSupport)
     {
        ...
        * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
        */
        if(LPUART_STAT_IDLE_MASK_U32 == (REG_READ32(LPUART_STAT_ADDR32(u8LPUART_Channel)) & LPUART_STAT_IDLE_MASK_U32))
        {
    ...
    */
    ...
    SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_00();
    {
        ...
        * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
        */
        REG_BIT_CLEAR32(LPUART_BAUD_ADDR32(u8LPUART_Channel), LPUART_BAUD_M10_MASK_U32);
        /**
        *
        * @violates @ref Lin_LPUART_c_REF_3 cast from unsigned int to pointer
        *
    ...
    {
        /* Create Sleep mode fake*/
        /* Disable LPUART Receiver and Transmitter Interrupt */
        /**
        *
        * @violates @ref Lin_LPUART_c_REF_3 cast from unsigned int to pointer
        *
    ...
    */
    ...
    if((uint8)E_OK == u8TmpReturn)
    {
        ...
        if (LIN_ENHANCED_CS == pPduInfoPtr->Cs)
        {
            ...
            bLPUART_ChecksumType = (boolean)TRUE;
        }
        else
        {
            /* Update LIN Checksum type to LIN_CLASSIC_CS */
            bLPUART_ChecksumType = (boolean)FALSE;
        }
        ...
        Lin_Lpuart_au8BufferLength[u8Channel] = ((uint8)3U) + ((pPduInfoPtr->Dl > LPUART_MAX_DATA_LENGTH_U8) ? LPUART_MAX_DATA_LENGTH_U8 : pPduInfoPtr->Dl);

        /* Write in the transmission buffer the synchronization byte and the PID */
        Lin_Lpuart_au8Buffer[u8Channel][LPUART_SYNCH_BYTE_OFFSET_U8] = LPUART_SYNC_BYTE_U8;
        ...
        else
        {
            ...
            SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_02();
            {
                ...
                */
                REG_READ32(LPUART_DATA_ADDR32(u8LPUART_Channel));
                /* Send break by toggling CTRL_SBK bit */
                /**
                *
                * @violates @ref Lin_LPUART_c_REF_3 cast from unsigned int to pointer
                *
                * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
                */
                REG_BIT_SET32(LPUART_CTRL_ADDR32(u8LPUART_Channel), LPUART_CTRL_SBK_MASK_U32);
                /**
                *
                * @violates @ref Lin_LPUART_c_REF_3 cast from unsigned int to pointer
                *
                * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
                */
                REG_BIT_CLEAR32(LPUART_CTRL_ADDR32(u8LPUART_Channel), LPUART_CTRL_SBK_MASK_U32);
...
FUNC (void, LIN_CODE) Lin_LPUART_SendResponse(CONST(uint8, AUTOMATIC) u8Channel, P2CONST(Lin_PduType, AUTOMATIC, LIN_APPL_CONST) pPduInfoPtr)
{
    ...
    /* Data */
    for(u8Loop = (uint8)0U; u8Loop < ((pPduInfoPtr->Dl > LPUART_MAX_DATA_LENGTH_U8) ? LPUART_MAX_DATA_LENGTH_U8 : pPduInfoPtr->Dl); u8Loop++)
    {
        /**
        * @violates @ref Lin_LPUART_c_REF_4 pointer arithmetic other
        *                than array indexing used
        */
        Lin_Lpuart_au8Buffer[u8Channel][LPUART_DATA_OFFSET_U8 + u8Loop] = pPduInfoPtr->SduPtr[u8Loop];
    }
    /* Checksum */
    u8Lin_FrameID = pPduInfoPtr->Pid;
    switch(u8Lin_FrameID)
    {
        case LPUART_MASTER_REQUEST_DIAGNOSTIC_ID_U8:
        case LPUART_SLAVE_RESPONSE_DIAGNOSTIC_ID_U8:
            Lin_Lpuart_au8Buffer[u8Channel][Lin_Lpuart_au8BufferLength[u8Channel] - ((uint8)1U)] = (uint8)Lin_LPUART_ChecksumCalc(u8Channel, LIN_CLASSIC_CS);
        break;
        default:
            if ((boolean)TRUE == bLPUART_ChecksumType)
            {
                /* Enhance checksum */
                Lin_Lpuart_au8Buffer[u8Channel][Lin_Lpuart_au8BufferLength[u8Channel] - ((uint8)1U)] = (uint8)Lin_LPUART_ChecksumCalc(u8Channel, LIN_ENHANCED_CS);
                bLPUART_ChecksumType = (boolean)TRUE;
            }
            else
            {
...
FUNC (Std_ReturnType, LIN_CODE) Lin_LPUART_GoToSleep(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    */
    ...
    if((uint8)E_OK == u8TmpReturn)
    {
        ...
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)2U] = LPUART_DATA_0_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)3U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)4U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)5U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)6U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)7U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)8U] = LPUART_DATA_255_U8;
        Lin_Lpuart_au8Buffer[u8Channel][(uint8)9U] = LPUART_DATA_255_U8;
...
FUNC (void, LIN_CODE) Lin_LPUART_WakeUp(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    u8WakeupByte = (uint8)((LPUART_MIN_WAKEUP_PULSE_LENGTH_U32/LPUART_BIT_LENGTH32(Lin_pcConfigPtr->pLin_Channel[u8Channel]->u32Baudrate)) + (uint8)1U);
    ...
    SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_08();
    {
        ...
        */
        ...
        /**
        * @violates @ref Lin_LPUART_c_REF_9 The right-hand operand shall not contain side effects
        */
        if((LIN_CH_SLEEP_STATE == Lin_au8LinChStatus[u8Channel]) && (0U != Lin_Lpuart_au8BufferLength[u8Channel]))
        {
            Lin_Lpuart_au8BufferLength[u8Channel] = (uint8)0U;
        }
...
FUNC(Lin_StatusType, LIN_CODE) Lin_LPUART_HardwareGetStatus(CONST(uint8, AUTOMATIC) u8Channel, P2VAR(uint8, AUTOMATIC, LIN_APPL_DATA) pu8LinSduPtr)
{
    ...
    VAR(uint8, AUTOMATIC) u8LinChFrameStatus;
    ...
    u8LinChFrameStatus = Lin_au8LinChFrameStatus[u8Channel];
    ...
    if(((uint8)0U) != u8Temp2)
    {
        ...
        if((LPUART_PID_BYTE_OFFSET_U8 + ((uint8)1U)) >= u8Temp1)
        {
            switch(u8ChannelFrameError)
            {
                ...
                case LIN_NO_ERROR:
                    if((LIN_TX_SLAVE_TO_SLAVE_COMMAND == u8TransmitHeaderCommand) && (LIN_TX_HEADER_COMPLETE_STATE == u8LinChFrameStatus))
                    {
                        u8TempReturn = LIN_TX_OK;
                    }
                    else if((LIN_TX_SLAVE_RES_COMMAND == u8TransmitHeaderCommand) && (LIN_TX_HEADER_COMPLETE_STATE == u8LinChFrameStatus))
                    {
        ...
        else if(u8Temp2 > u8Temp1)
        {
            switch(u8ChannelFrameError)
            {
                ...
                case LIN_NO_ERROR:
                    ...
                    {
                        if (LIN_RX_ONGOING_STATE == u8LinChFrameStatus)
                        {
                            u8TempReturn = LIN_RX_BUSY;
                        }
                    }
                    else if ((LIN_TX_SLAVE_TO_SLAVE_COMMAND == u8TransmitHeaderCommand) && (LIN_TX_COMPLETE_STATE == u8LinChFrameStatus))
                    {
                        u8TempReturn = LIN_TX_OK;
                    }
                    else
            ...
            */
            ...
            if ((uint32)0U != u32Lin_Status)
            {
                ...
                if((LPUART_STAT_TDRE_MASK_U32 == (u32Lin_Status & LPUART_STAT_TDRE_MASK_U32)) && (LPUART_CTRL_TIE_MASK_U32 == (u32Lin_EnabledInterrupts & LPUART_CTRL_TIE_MASK_U32)))
                {
                    ...
                    * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
                    */
                    REG_WRITE32(LPUART_DATA_ADDR32(u8LPUART_Channel), Lin_Lpuart_au8Buffer[u8LogicalChannel][u8BufferPtr]);
