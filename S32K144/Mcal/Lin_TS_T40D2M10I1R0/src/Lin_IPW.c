...
* external identifiers.
*/
#define LIN_AR_RELEASE_MINOR_VERSION_IPW_C     3
/*
* @violates @ref Lin_Ipw_c_REF_3 The compiler/linker shall be checked to
* ensure that 31 character signifiance and case sensitivity are supported for
* external identifiers.
*/
#define LIN_AR_RELEASE_REVISION_VERSION_IPW_C  1
...
#if (LIN_IPW_VENDOR_ID_C != LIN_FLEXIO_VENDOR_ID)
    #error "Lin_IPW.c and Lin_FlexIO.h have different vendor ids"
#endif

/* Check if current file and Lin_FlexIO header file are of the same Autosar version */
#if ((LIN_AR_RELEASE_MAJOR_VERSION_IPW_C    != LIN_FLEXIO_AR_RELEASE_MAJOR_VERSION) || \
     (LIN_AR_RELEASE_MINOR_VERSION_IPW_C    != LIN_FLEXIO_AR_RELEASE_MINOR_VERSION) || \
     (LIN_AR_RELEASE_REVISION_VERSION_IPW_C != LIN_FLEXIO_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Lin_IPW.c and Lin_FlexIO.h are different"
#endif
/* Check if current file and Lin_FlexIO header file are of the same Software version */
#if ((LIN_SW_MAJOR_VERSION_IPW_C != LIN_FLEXIO_SW_MAJOR_VERSION) || \
     (LIN_SW_MINOR_VERSION_IPW_C != LIN_FLEXIO_SW_MINOR_VERSION) || \
     (LIN_SW_PATCH_VERSION_IPW_C != LIN_FLEXIO_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Lin_IPW.c and Lin_FlexIO.h are different"
#endif
#endif
/*==================================================================================================
...
FUNC (Std_ReturnType, LIN_CODE) Lin_Ipw_CheckWakeup(CONST(uint8, AUTOMATIC) u8Channel)
{
    VAR(Std_ReturnType, AUTOMATIC) u8TempReturn = (uint8)E_NOT_OK;
     ...
     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->eChannelType))
     {
        u8TempReturn = Lin_LPUART_CheckWakeup(u8Channel);
    }
    else
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        u8TempReturn = Lin_FlexIO_CheckWakeup(u8Channel);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC (void, LIN_CODE) Lin_Ipw_InitChannel(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        Lin_FlexIO_InitChannel(u8Channel);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC(Std_ReturnType, LIN_CODE) Lin_Ipw_SendHeader(CONST(uint8, AUTOMATIC) u8Channel, P2CONST(Lin_PduType, AUTOMATIC, LIN_APPL_CONST) pcPduInfoPtr)
{
    VAR(Std_ReturnType, AUTOMATIC) u8TempReturn = (uint8)E_NOT_OK;
     ...
     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->eChannelType))
     {
        u8TempReturn = Lin_LPUART_SendHeader(u8Channel, pcPduInfoPtr);
    }
    else
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        u8TempReturn = Lin_FlexIO_SendHeader(u8Channel, pcPduInfoPtr);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC(void, LIN_CODE) Lin_Ipw_SendResponse(CONST(uint8, AUTOMATIC) u8Channel, P2CONST(Lin_PduType, AUTOMATIC, LIN_APPL_CONST) pcPduInfoPtr)
{
    ...
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        Lin_FlexIO_SendResponse(u8Channel, pcPduInfoPtr);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC (Std_ReturnType, LIN_CODE) Lin_Ipw_GoToSleep(CONST(uint8, AUTOMATIC) u8Channel)
{
    VAR(Std_ReturnType, AUTOMATIC) u8TempReturn = (uint8)E_NOT_OK;
     ...
     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->eChannelType))
     {
        u8TempReturn = Lin_LPUART_GoToSleep(u8Channel);
    }
    else
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        u8TempReturn = Lin_FlexIO_GoToSleep(u8Channel);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC (Std_ReturnType, LIN_CODE) Lin_Ipw_GoToSleepInternal(CONST(uint8, AUTOMATIC) u8Channel)
{
    VAR(Std_ReturnType, AUTOMATIC) u8TempReturn = (uint8)E_NOT_OK;
     ...
     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->eChannelType))
     {
        u8TempReturn = Lin_LPUART_GoToSleepInternal(u8Channel);
    }
    else
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        u8TempReturn = Lin_FlexIO_GoToSleepInternal(u8Channel);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC (Lin_StatusType, LIN_CODE) Lin_Ipw_HardwareGetStatus(CONST(uint8, AUTOMATIC) u8Channel, P2VAR(uint8, AUTOMATIC, LIN_APPL_DATA) pu8LinSduPtr)
{
    VAR(Lin_StatusType,AUTOMATIC) u8TempReturn = LIN_NOT_OK;
     ...
     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->eChannelType))
     {
        u8TempReturn = Lin_LPUART_HardwareGetStatus(u8Channel, pu8LinSduPtr);
    }
    else
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        u8TempReturn = Lin_FlexIO_HardwareGetStatus(u8Channel, pu8LinSduPtr);
    #else
        /* Do nothing */
    #endif
    }
...
FUNC (void, LIN_CODE) Lin_Ipw_WakeUpInternal(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    {
    #if (LIN_FLEXIO_USED == STD_ON)
        Lin_FlexIO_WakeupInternal(u8Channel);
    #else
        /* Do nothing */
    #endif
    }
