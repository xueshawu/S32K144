...
#define LIN_AR_RELEASE_MINOR_VERSION_C       3
...
*/
#define LIN_AR_RELEASE_REVISION_VERSION_C    1
...
* or functions at file scope shall have internal linkage unless external linkage is required.
*/
VAR(uint8,LIN_VAR) Lin_u8LinDrvStatus = LIN_UNINIT;
    ...
    else
    {
        ...
        for(u8Lin_Hw_ChLoop=(uint8)0U;u8Lin_Hw_ChLoop<LIN_HW_MAX_AVAILABLE_MODULES;u8Lin_Hw_ChLoop++)
        {
         ...
         for(u8Lin_ChLoop=(uint8)0U; u8Lin_ChLoop < LIN_HW_MAX_MODULES; u8Lin_ChLoop++)
         {
            ...
            Lin_au8LinChStatus[u8Lin_ChLoop] = LIN_CH_SLEEP_STATE;
            /* Set LIN channel frame operation status to LIN_CH_NOT_READY_STATE */
            Lin_au8LinChFrameStatus[u8Lin_ChLoop] = LIN_CH_NOT_READY_STATE;
             /* Header status flag */
        ...
        else
        {
            ...
            if (LIN_CH_SLEEP_STATE == Lin_au8LinChStatus[Channel])
            {
                ...
                if ((uint8)TRUE == Lin_Ipw_CheckWakeup(Channel))
                {
                    ...
                    LinIf_WakeupConfirmation(Lin_pChannelConfigPtr[Channel]->pChannelConfigPC->LinChannelEcuMWakeupSource);
                 }
            }
            else
            {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                Lin_au8LinChStatus[Channel] = LIN_CH_SLEEP_PENDING;
             }
        }
    }
    return u8TempReturn;
