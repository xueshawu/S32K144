...
* external identifiers.
*/
#define LIN_AR_RELEASE_MINOR_VERSION_NONASR_C       3
/*
* @violates @ref Lin_NonASR_c_REF_2 The compiler/linker shall be checked to
* ensure that 31 character signifiance and case sensitivity are supported for
* external identifiers.
*/
#define LIN_AR_RELEASE_REVISION_VERSION_NONASR_C    1
...
#if (LIN_VENDOR_ID_NONASR_C != LIN_FLEXIO_VENDOR_ID)
    #error "Lin_NonASR.c and Lin_FlexIO.h have different vendor ids"
#endif

/* Check if current file and Lin_FlexIO header file are of the same Autosar version */
#if ((LIN_AR_RELEASE_MAJOR_VERSION_NONASR_C    != LIN_FLEXIO_AR_RELEASE_MAJOR_VERSION) || \
     (LIN_AR_RELEASE_MINOR_VERSION_NONASR_C    != LIN_FLEXIO_AR_RELEASE_MINOR_VERSION) || \
     (LIN_AR_RELEASE_REVISION_VERSION_NONASR_C != LIN_FLEXIO_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Lin_NonASR.c and Lin_FlexIO.h are different"
#endif
/* Check if current file and Lin_FlexIO header file are of the same Software version */
#if ((LIN_SW_MAJOR_VERSION_NONASR_C != LIN_FLEXIO_SW_MAJOR_VERSION) || \
     (LIN_SW_MINOR_VERSION_NONASR_C != LIN_FLEXIO_SW_MINOR_VERSION) || \
     (LIN_SW_PATCH_VERSION_NONASR_C != LIN_FLEXIO_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Lin_NonASR.c and Lin_FlexIO.h are different"
#endif

/* Check if current file and Reg_eSys_FlexIO header file are of the same vendor */
#if (LIN_VENDOR_ID_NONASR_C != REG_ESYS_FLEXIO_VENDOR_ID)
    #error "Lin_NonASR.c and Reg_eSys_FlexIO.h have different vendor ids"
#endif

/* Check if current file and Reg_eSys_FlexIO header file are of the same Autosar version */
#if ((LIN_AR_RELEASE_MAJOR_VERSION_NONASR_C    != REG_ESYS_FLEXIO_AR_RELEASE_MAJOR_VERSION) || \
     (LIN_AR_RELEASE_MINOR_VERSION_NONASR_C    != REG_ESYS_FLEXIO_AR_RELEASE_MINOR_VERSION) || \
     (LIN_AR_RELEASE_REVISION_VERSION_NONASR_C != REG_ESYS_FLEXIO_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Lin_NonASR.c and Reg_eSys_FlexIO.h are different"
#endif
/* Check if current file and Reg_eSys_FlexIO header file are of the same Software version */
#if ((LIN_SW_MAJOR_VERSION_NONASR_C != REG_ESYS_FLEXIO_SW_MAJOR_VERSION) || \
     (LIN_SW_MINOR_VERSION_NONASR_C != REG_ESYS_FLEXIO_SW_MINOR_VERSION) || \
     (LIN_SW_PATCH_VERSION_NONASR_C != REG_ESYS_FLEXIO_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Lin_NonASR.c and Reg_eSys_FlexIO.h are different"
#endif
#endif
...
FUNC(Std_ReturnType, LIN_CODE) Lin_SetClockMode(VAR(Lin_ClockModesType, AUTOMATIC) ClockMode)
{
    ...
    else
    {
        for(u8chLoop=(uint8)0U; u8chLoop < LIN_HW_MAX_MODULES; u8chLoop++)
        {
            ...
            {
    #if (LIN_FLEXIO_USED == STD_ON)
                     if(LIN_LPUART_CHANNEL == (Lin_pChannelConfigPtr[u8chLoop]->pChannelConfigPC->eChannelType))
                     {
    #endif
                ...
                else
                {
                    ...
                    {
                        /* Configure Tx timer */
                        /* Configure 8-bit transfer with baud rate of divide by 4 of the FlexIO clock */
                        /**
                        * @violates @ref Lin_NonASR_c_REF_3 cast from unsigned int to pointer
                        *
                        * @violates @ref Lin_NonASR_c_REF_7 cast should not be performed
                        */
                        REG_WRITE32(FLEXIO_TIMCMPn_ADDR32((uint32)(Lin_pChannelConfigPtr[u8chLoop]->pFlexIOConfigPC->u32TxTimerId)), \
                                                         ((uint32)(Lin_pChannelConfigPtr[u8chLoop]->u32Lin_BaudRate_RegValue) | (uint32)FLEXIO_TIMCMP_BITS_NUMBER_8_U32) \
                                   );
                        /* Configure Rx timer */
                        /* Configure 8-bit transfer with baud rate of divide by 4 of the FlexIO clock */
                        /**
                        ...
                        * @violates @ref Lin_NonASR_c_REF_7 cast should not be performed
                        */
                        REG_WRITE32(FLEXIO_TIMCMPn_ADDR32((uint32)(Lin_pChannelConfigPtr[u8chLoop]->pFlexIOConfigPC->u32RxTimerId)), \
                                                         ((uint32)(Lin_pChannelConfigPtr[u8chLoop]->u32Lin_BaudRate_RegValue) \
                    ...
                    else
                    {
                        ...
                        */
                        REG_WRITE32(FLEXIO_TIMCMPn_ADDR32((uint32)(Lin_pChannelConfigPtr[u8chLoop]->pFlexIOConfigPC->u32TxTimerId)), \
                                                         ((uint32)(Lin_pChannelConfigPtr[u8chLoop]->u32Lin_BaudRate_RegValue_Alternate) | (uint32)FLEXIO_TIMCMP_BITS_NUMBER_8_U32) \
                                   );
                        /* Configure Rx timer */
                        /* Configure 8-bit transfer with baud rate of divide by 4 of the FlexIO clock */
                        /**
                        ...
                        */
                        REG_WRITE32(FLEXIO_TIMCMPn_ADDR32((uint32)(Lin_pChannelConfigPtr[u8chLoop]->pFlexIOConfigPC->u32RxTimerId)), \
                                                         ((uint32)(Lin_pChannelConfigPtr[u8chLoop]->u32Lin_BaudRate_RegValue_Alternate) \
                                                         | (uint32)FLEXIO_TIMCMP_BITS_NUMBER_8_U32) \
                                   );
                    }
                }
