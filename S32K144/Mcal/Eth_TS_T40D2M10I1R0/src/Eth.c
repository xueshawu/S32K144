...
==================================================================================================*/
static FUNC(uint8, ETH_CODE) Eth_GetQueueIndexByPriority ( \
...
/**
* @brief Get queue from configured priorities
*/
static FUNC(uint8, ETH_CODE) Eth_GetQueueIndexByPriority ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) Priority \
...
{
    VAR(uint8, AUTOMATIC) u8ReturnValue = 0U;   /* Return value, default is queue 0 */
    VAR(uint8, AUTOMATIC) u8Idx;                /* Index variable */
    
    if ( (NULL_PTR != Eth_InternalCfgPtr) && (NULL_PTR != ((Eth_InternalCfgPtr->pController)[u8CtrlIdx])->pTxPrioFifoMapping) )
    {
        for (u8Idx = 0U; u8Idx < ETH_MAX_NUM_PRIORITY; u8Idx++)
        {
            if ((*(((Eth_InternalCfgPtr->pController)[u8CtrlIdx])->pTxPrioFifoMapping))[u8Idx].u8Priority == Priority)
            {
                u8ReturnValue = (*(((Eth_InternalCfgPtr->pController)[u8CtrlIdx])->pTxPrioFifoMapping))[u8Idx].u8TxFifoIdx;
            }
            else
            {
                /* Prevent MISRA-C warning/error */
            }
        }
    }
    else
...
FUNC(void, ETH_CODE) Eth_Init(P2CONST(Eth_ConfigType, AUTOMATIC, ETH_APPL_CONST) CfgPtr)
{
    ...
    #else
            (void)CfgPtr; /* to avoid compiler warning*/
    #endif
                                                ...
                                                )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#endif /* ETH_DEV_ERROR_DETECT  */
                Eth_Ipw_GetCounterValues(CtrlIdx, CounterPtr);
                eReturnStatus = (VAR(Std_ReturnType, AUTOMATIC))E_OK;
#if STD_ON == ETH_DEV_ERROR_DETECT
            } 
        }
                                              ...
                                              )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#endif /* ETH_DEV_ERROR_DETECT  */
                Eth_Ipw_GetRxStats(CtrlIdx, RxStats);
                eReturnStatus = (VAR(Std_ReturnType, AUTOMATIC)) E_OK;
#if STD_ON == ETH_DEV_ERROR_DETECT
            } 
        }
...
/**
* @brief        Read the status of a controller
* @detail       Returns the following list according to IETF RFC1213, where the maximal possible 
*               value shall denote an invalid value, e.g. if this counter is not available:
*               1. TxNumberOfOctets
*               2. TxNUcastPkts
*               3. TxUniCastPkts
                                              ...
                                              )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#endif /* ETH_DEV_ERROR_DETECT  */
                Eth_Ipw_GetTxStats(CtrlIdx, TxStats);
                eReturnStatus = (VAR(Std_ReturnType, AUTOMATIC)) E_OK;
#if STD_ON == ETH_DEV_ERROR_DETECT
            } 
        }
...
* @detail       Returns the following list according to IETF RFC1213, where the maximal possible 
*               value shall denote an invalid value, e.g. if this counter is not available:
*               1. TxDroppedNoErrorPkts
*               2. TxDroppedErrorPkts
*               3. TxDeferredTrans
*               4. TxSingleCollision
*               5. TxMultipleCollision
*               6. TxLateCollision
*               7. TxExcessiveCollison
* @api
* @param[in]    CtrlIdx Index of the controller which shall be read the status register.
                                              ...
                                              )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#endif /* ETH_DEV_ERROR_DETECT  */
                Eth_Ipw_GetTxErrorCounterValues(CtrlIdx, TxErrorCounterValues);
                eReturnStatus = (VAR(Std_ReturnType, AUTOMATIC)) E_OK;
#if STD_ON == ETH_DEV_ERROR_DETECT
            } 
        }
...
#endif /* ETH_GETTXERRORCOUNTERVALUES_API */
    ...
    else
    {
        if(ETH_STATE_INIT != Eth_CtrlState[CtrlIdx])
        {
            (void)Det_ReportError ( \
                (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                ETH_SID_GETCURRENTTIME, ETH_E_UNINIT \
                                  );
        }
        else
        {
    ...
    else
    {
        if(ETH_STATE_INIT != Eth_CtrlState[CtrlIdx])
        {
            (void)Det_ReportError ( \
                (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                ETH_SID_ENABLEEGRESSTIMESTAMP, ETH_E_UNINIT \
                                  );
        }
        else
        {
    ...
    else
    {
        if(ETH_STATE_INIT != Eth_CtrlState[CtrlIdx])
        {
            (void)Det_ReportError ( \
                (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                ETH_SID_GETEGRESSTIMESTAMP, ETH_E_UNINIT \
                                  );
        }
        else
        {
    ...
    else
    {
        if(ETH_STATE_INIT != Eth_CtrlState[CtrlIdx])
        {
            (void)Det_ReportError ( \
                (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                ETH_SID_GETINGRESSTIMESTAMP, ETH_E_UNINIT \
                                  );
        }
        else
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        if(Priority >= ETH_MAX_NUM_PRIORITY)
                        {
                            /* Priority shall comply IEEE P802.1p */
                            (void)Det_ReportError ( \
                                (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                                ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                                ETH_SID_PROVIDETXBUFFER, ETH_E_INV_PARAM \
                                              );
                        }
                        else
                        {
                            ...
                            u8QueueIdx = Eth_GetQueueIndexByPriority(CtrlIdx, Priority);
    #if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
                            /** @violates @ref Eth_c_REF_8 MISRA rule 17.4 */
                            /** @violates @ref Eth_c_REF_13 MISRA rule 12.1 */
                            /** @violates @ref Eth_c_REF_6 MISRA rule 1.2 */
                            u32MaxLenByte = ((uint32)Eth_Ipw_GetTxBufLength(CtrlIdx, u8QueueIdx) * (uint32)Eth_Ipw_GetTxBufNumber(CtrlIdx, u8QueueIdx)) - \
                                            (VAR(uint32, AUTOMATIC))14U;
    #else
                            /** @violates @ref Eth_c_REF_8 MISRA rule 17.4 */
                            /** @violates @ref Eth_c_REF_13 MISRA rule 12.1 */
                            /** @violates @ref Eth_c_REF_6 MISRA rule 1.2 */
                            u32MaxLenByte = (uint32)Eth_Ipw_GetTxBufLength(CtrlIdx, u8QueueIdx) - (VAR(uint32, AUTOMATIC))14U;
    #endif
                            ...
                            *LenBytePtr = (VAR(uint16,AUTOMATIC))u32MaxLenByte;
                            ...
                            {   /* Good, the requested size fits into buffer size */
                                ...
                                /** @violates @ref Eth_c_REF_6 MISRA rule 1.2 */
                                if(((VAR(boolean, AUTOMATIC))TRUE) == Eth_Ipw_ProvideBufferDataArea(CtrlIdx, u8QueueIdx, BufIdxPtr, BufPtr, &u16TempLenByte))
...
#if STD_ON == ETH_DEV_ERROR_DETECT
    /*Temporary variable used to avoid compiler warning */
    VAR(uint32, AUTOMATIC) TxLogicalBufTotal = ETH_CFG_TXLOGICALBUFTOTAL(CtrlIdx);

    /** @violates @ref Eth_c_REF_12 MISRA rule 10.1 */
    /** @violates @ref Eth_c_REF_5 MISRA rule 13.7 */
    if (0 == TxLogicalBufTotal)
    {
       (void)Det_ReportError ( \
          (VAR(uint8, AUTOMATIC)) \
                                 ...
                                 )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if(FifoIdx >= ETH_QUEUE_CFG)
                {
                    /* FifoIdx shall not exceed the number of FIFO that hardware can support */
                    (void)Det_ReportError ( \
                        (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                        ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                        ETH_SID_RECEIVE, ETH_E_INV_PARAM \
                                        );
                }
                else
                {
                    ...
                    if((VAR(boolean, AUTOMATIC))FALSE == ETH_CFG_ENABLERXINTERRUPT(CtrlIdx))
                    {
                        ...
                        *RxStatusPtr = Eth_Ipw_ReportReception(CtrlIdx, FifoIdx, (VAR(boolean, AUTOMATIC))FALSE);
