...
* even with the array indexing arithmetic - this is used in a post build configuration variant
...
*
* @section Eth_Enet_c_REF_21
* Violates MISRA 2004 Required Rule 10.6, A "U" suffix shall be applied to all constants of unsigned type.
*
...
*
* @section Eth_Enet_c_REF_23
* Violates MISRA 2004 Advisory Rule 12.1, Limited dependence should be placed on C's operator
...
* shall be tested. 
*
* @section Eth_Enet_c_REF_26
* Violates MISRA 2004 Required Rule 9.1, All automatic variables shall have been assigned a value
* before being used. 
*
* @section Eth_Enet_c_REF_27
* Violates MISRA 2004 Required Rule 8.1, Functions shall have prototype declarations and the prototype shall be visible
* at both function definition and call. 
*
* @section [global]
...
/** @violates @ref Eth_Enet_c_REF_21 MISRA rule 10.6 */
...
#include "Eth_MemMap.h"
    ...
    *          for the errata 6358.
    */
    static VAR(boolean, ETH_VAR) Eth_ENET_bMoreDataForTx[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG] = {(VAR(boolean, AUTOMATIC))FALSE};
...
*/
static VAR(uint32, ETH_VAR) Eth_ENET_u32TxBufStartAddr[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG]={{0U}};
...
*/
static VAR(uint32, ETH_VAR) Eth_ENET_u32RxBufStartAddr[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG]={{0U}};
...
*               allocated buffers.
*/
static VAR(Eth_BufIdxType, ETH_VAR) Eth_ENET_u32SearchTxBufFrom[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG] = {{0U}};
...
*               buffer of frame. */
static VAR(Eth_BufIdxType, ETH_VAR) Eth_au32TxBdOfBuf[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG * ETHTXBUFNUM];
...
static VAR(volatile uint8, ETH_VAR) Eth_au8TxBufFlags[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG * ETHTXBUFNUM];
...
static VAR(Eth_BufIdxType, ETH_VAR) Eth_ENET_au32TxBufGroup[ETH_MAXCTRLS_SUPPORTED][ETHTXBUFNUM];
...
* @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 
*/
VAR(volatile Eth_BufIdxType, ETH_VAR) Eth_u32ActiveRxBuf[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG] = {{0U}};
...
* @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 
*/
VAR(volatile Eth_BufIdxType, ETH_VAR) Eth_u32ActiveTxBD[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG] = {{0U}};
...
*/
/** @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 */
VAR(Eth_BufIdxType, ETH_VAR) Eth_u32LockedTxBufCount[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG] = {{0U}};
...
LOCAL_INLINE FUNC(void, ETH_CODE) Eth_Enet_Write16TxBufMem  ( \
                    ...
                    CONST(uint32, AUTOMATIC) u32BufIdx, \
...
LOCAL_INLINE FUNC(void, ETH_CODE) Eth_Enet_WriteRxBDBitsA   ( \
                CONST(uint32, AUTOMATIC) u32Base, \
                CONST(Eth_BufIdxType, AUTOMATIC) u32BufNum, \
...
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ComputeTxBufAddr   ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                CONST(uint8, AUTOMATIC) u8QueueIdx, \
                CONST(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                                );
/*Compute Rx_BufAddr*/
...
static FUNC(void, ETH_CODE)Eth_Enet_ClearRxBuffer ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        CONST(uint8, AUTOMATIC) u8QueueIdx, \
                        CONST(Eth_BufIdxType, AUTOMATIC) u32Buf \
                                                  );
static FUNC(void, ETH_CODE) Eth_Enet_GetRxBufferData ( \
                    CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                    CONST(uint8, AUTOMATIC) u8QueueIdx, \
                    CONST(Eth_BufIdxType, AUTOMATIC) u32Buf, \
                    CONST(Eth_BufIdxType, AUTOMATIC) u32MultiEnd, \
...
static FUNC(Eth_Enet_enRxBufferType, ETH_CODE) Eth_Enet_GetRxBufferType ( \
                    ...
                    CONST(uint8, AUTOMATIC) u8QueueIdx, \
                    CONST(Eth_BufIdxType, AUTOMATIC) u32Buf, \
                    CONST(Eth_BufIdxType, AUTOMATIC) u32BufCtrMax, \
                    CONSTP2VAR(Eth_BufIdxType, AUTOMATIC, ETH_APPL_DATA) pMultiEnd \
                                                                            );

static FUNC(void, ETH_CODE) Eth_Enet_GiveBackTxBuffer ( \
                    CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                    CONST(uint8, AUTOMATIC) u8QueueIdx, \
                    VAR(Eth_BufIdxType, AUTOMATIC) u32Buf
                                                      );
#if STD_ON == ETH_UPDATE_PHYS_ADDR_FILTER
...
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufStartIndex ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                          );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufStartIndex ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                          );

LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_GetTxBDBufStartAddress ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                             );

LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_GetRxBDBufStartAddress ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                             );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufIdxMax( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                            );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufAbsIdx( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx, \
                        VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                            );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxLogicalBufIdx( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx, \
                        VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                            );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufIdxMax( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                            );

LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_GetRxBufLength ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx
                                                             );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufNumber ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                             );

LOCAL_INLINE FUNC(uint8, ETH_CODE) Eth_Enet_GetQueueIndexByTxBufIdx ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                          );

LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufOffset ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                                );
/*==================================================================================================
...
*                by setting of 0x00U to GAUR and GALR registers.
...
* @param[in]      u32BufNum Index of the buffer descriptor to be written
                                                            ...
                                                            )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA))(u32Base + (u32BufNum * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE))) = u32Data;
}
                                                            ...
                                                            )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA))(u32Base + (u32BufNum * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE) + 8U)) = u32Data;
}
                                                            ...
                                                            )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA)) (u32Base + (u32BufNum * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE) + 4U)) = u32Data;
}
...
* @param[in]      u32Base Starting address of the Receive buffer descriptors ring
* @param[in]      u32BufNum Index of the buffer descriptor to be read
* @return         Read status bits (upper 16 bits) and buffer length
*                 (lower 16 bits)
* @note           This function is an inline function.
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ReadRxBDBitsA  (CONST(uint32, AUTOMATIC) u32Base, CONST(Eth_BufIdxType, AUTOMATIC) u32BufNum)
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    return *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA)) (u32Base + (u32BufNum * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE)));
}
/*================================================================================================*/
/**
* @brief          Reads the status bits from the Receive buffer descriptor
...
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ReadRxBDBitsB (CONST(uint32, AUTOMATIC) u32Base, CONST(Eth_BufIdxType, AUTOMATIC) u32BufNum)
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    return *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA)) (u32Base + (u32BufNum * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE) + 8U));
}
#if STD_ON == ETH_GLOBALTIME_SUPPORT
/*================================================================================================*/
/**
...
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ReadTxBDBitsB (CONST(uint32, AUTOMATIC) u32Base, CONST(Eth_BufIdxType, AUTOMATIC) u32BufNum)
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
...
* @param[in]      u32BufIdx Offset to be added to base address before the write
...
* @implements     Eth_Enet_Write16TxBufMem_Activity
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(void, ETH_CODE) Eth_Enet_Write16TxBufMem  ( \
                                CONST(uint32, AUTOMATIC) u32Base, \
                                                            ...
                                                            )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    *((P2VAR(volatile uint16, AUTOMATIC, ETH_APPL_DATA))(u32Base + (u32BufIdx))) = u16Data;
}
...
* @param[in]    u32BufIdx Index of buffer we need address of.
...
{
    VAR(uint32, AUTOMATIC) u32ReturnVal;       /* Return variable */
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    u32ReturnVal = ( Eth_ENET_u32TxBufStartAddr[u8CtrlIdx][u8QueueIdx] + ((VAR(uint32, AUTOMATIC))ETH_CFG_CTRLTXBUF0LENBYTE( u8CtrlIdx) * u32BufIdx));
...
{
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    return ( Eth_ENET_u32RxBufStartAddr[u8CtrlIdx][u8QueueIdx] + ((uint32)Eth_Enet_GetRxBufLength(u8CtrlIdx, u8QueueIdx) * (uint32)u32BufIdx));
}
...
* @param[in]    u32BufIdx Index of transmit buffer to be read
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ReadTxBDTimestampPtr   ( \
                                                    CONST(uint32, AUTOMATIC) u32BdAddress, \
                                                                    ...
                                                                    )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    return *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA)) (u32BdAddress + (u32BufIdx * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE) + ENET_TXBD_TS_ADDR16));
}
/*============================================================================*/
/**
* @brief        Readout the timestamp from specific TxBD
...
* @param[in]    u8CtrlIdx Index of controller we are correcting
* @param[in]    u32BufIdx Index of received buffer to be read
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_ReadRxBDTimestampPtr ( \
                                                    CONST(uint32, AUTOMATIC) u32BdAddress, \
                                                          ...
                                                          )
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
    return *((P2VAR(volatile uint32, AUTOMATIC, ETH_APPL_DATA)) (u32BdAddress + (u32BufIdx * (VAR(uint32, AUTOMATIC))ETH_ENET_LLD_BD_SIZE) + ENET_RXBD_TS_ADDR16));
}
#endif
...
* @param[in]    u32Buf Index of the buffer to be cleared
*/

static FUNC(void, ETH_CODE)Eth_Enet_ClearRxBuffer (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx, CONST(Eth_BufIdxType, AUTOMATIC) u32Buf)
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNum;       /* Configured number of buffers */
    ...
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    /* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
    u32BdAddress = Eth_Enet_GetRxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);

    /* Get the number of configured buffers */
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    u32BufNum = Eth_Enet_GetRxBufNumber(u8CtrlIdx, u8QueueIdx);
    /* Clear the buffer */
    ...
    if(u32BufNum  == (u32Buf + 1U))
    {   /* The last buffer in the ring */
        /* Set the wrap bit */
        u32ManipulationVar |= ENET_RXBD_W_U32;
    }
    /* Write the assembled value into the descriptor */
    Eth_Enet_WriteRxBDBitsA(u32BdAddress, u32Buf, u32ManipulationVar); 
    
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
    if ((uint8)E_OK == Mcl_CacheClearMultiLines(MCL_LMEM_CACHE_PS, u32BdAddress, ETH_TX_BUF_MEM_SIZE))
    {
    #endif
  #endif
#endif
...
* @param[in]     u32Buf Buffer to be read
                                                ...
                                                )
{
    ...
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufTotal = Eth_Enet_GetRxBufNumber(u8CtrlIdx, u8QueueIdx);
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufCtrMax = u32BufTotal - 1U;
...
#if STD_ON == ETH_USE_RX_FRAMES_WRAP
      VAR(uint32, AUTOMATIC) u32BufLength = 0U;
#endif
#else
      VAR(uint32, AUTOMATIC) u32BufLength = 0U;
    ...
    /** @violates @ref Eth_Enet_c_REF_20 MISRA rule 10.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    VAR(uint32, AUTOMATIC) u32BDAddress = Eth_Enet_GetRxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
#if STD_ON == ETH_USE_MULTIBUFFER_RX_FRAMES  
#if STD_ON == ETH_USE_RX_FRAMES_WRAP
    u32BufLength = Eth_Enet_GetRxBufLength(u8CtrlIdx, u8QueueIdx);
#endif
#else
    u32BufLength = Eth_Enet_GetRxBufLength(u8CtrlIdx, u8QueueIdx);
#endif /* ETH_USE_MULTIBUFFER_RX_FRAMES */
    /* Read buf.status (A) */
    u32ManipulationVar = Eth_Enet_ReadRxBDBitsA(u32BDAddress, u32MultiEnd);
...
#else /* ETH_USE_MULTIBUFFER_RX_FRAMES */
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    if( u16FrameLength > (uint16)u32BufLength )
    {   /* The length must be incorrect, it is greater than maximal allowed length */
        /* Return maximal length instead */
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        *pLength = (VAR(uint16, AUTOMATIC)) u32BufLength;
    }
#endif /* ETH_USE_MULTIBUFFER_RX_FRAMES */
    ...
    u32ManipulationVar |= Eth_Enet_ReadRxBDBitsB(u32BDAddress, u32Buf) & 0x8600E037U;
    /* Return the status bits */
    *pStatusPtr = u32ManipulationVar;
    if (u32Buf>u32MultiEnd)
    {
    /*Wrap occurred */
    #if STD_ON == ETH_USE_RX_FRAMES_WRAP
        /* Check whether data should be copied to leading or trailing area -
           chose which copy will be shorter: either buffer ring start to "u32MultiEnd",
           or "u32Buf" to buffer ring end. */
        if ((u32BufCtrMax-u32Buf) > u32MultiEnd)
        { /* Copy from the start of the buffer ring to the trailing area because it is shorter */
            /* Setup source address */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32srcAddr = Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, 0U);
            /* Setup destination address */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32destAddr = Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, u32BufCtrMax + 1U);
            /* Store the data pointer */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32ManipulationVar = Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, u32Buf);
            /* Return the pointer */
            /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
                @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
            */
            ...
            *pDataPtr = (P2VAR(Eth_DataType, AUTOMATIC, ETH_APPL_DATA)) u32ManipulationVar;
            ...
            u32ManipulationVar = ((VAR(uint32, AUTOMATIC))(*pLength) + 4U) \
                - (((uint32)u32BufTotal - (VAR(uint32, AUTOMATIC))u32Buf) \
                * (uint32)u32BufLength);
        }
        else
        { /* Copy from the end of the buffer ring to the leading area because it is shorter */
            ...
            /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32srcAddr = Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, u32Buf);
            /* Set destination address */
            /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32destAddr = Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, 0U) \
                            - (((uint32)u32BufTotal - (VAR(uint32, AUTOMATIC))u32Buf) * (uint32)u32BufLength);
            /* Return the pointer */
            /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
                @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
            */
            ...
            *pDataPtr = (P2VAR(Eth_DataType, AUTOMATIC, ETH_APPL_DATA)) u32destAddr;
            /* Setup count */
            /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
            u32ManipulationVar = ((uint32)u32BufTotal - (VAR(uint32, AUTOMATIC))u32Buf) * (uint32)u32BufLength;
        }
...
* @param[in]     u32Buf Index of the examined buffer
* @param[in]     u32BufCtrMax Number of the Rx buffers
...
{
    VAR(uint32, AUTOMATIC) u32ManipulationVar;
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufCtr;
    ...
    VAR(uint32, AUTOMATIC) u32BDAddress;
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_20 MISRA rule 10.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    ...
    u32ManipulationVar = Eth_Enet_ReadRxBDBitsA(u32BDAddress, u32Buf);

    (void) u8CtrlIdx; /*to avoid compiler warning*/
    ...
    { /* Buffer is full */
        ...
        { /* Buffer contains truncated frame */
            ...
            *pMultiEnd = u32Buf; /* It is finished in current buffer,
        ...
        {   /* Buffer contains only part of the frame */
            ...
            do
            {
                ...
                if(u32BufCtrMax == u32BufCtr) /* Check if the counter wraps */
                { /* Yes, it wraps  */
                    u32BufCtr = 0U;
                }
                else
                ...
                if(u32BufCtr == u32Buf) /* Check  the overflow */
                ...
                { /* No overflow detected */
                    u32ManipulationVar = Eth_Enet_ReadRxBDBitsA(u32BDAddress, u32BufCtr);
                    /* Check empty state */
                    if(ENET_RXBD_E_U32 == (u32ManipulationVar & ENET_RXBD_E_U32))
...
* @param[in]     u32Buf DA index of the buffer to be unlocked
...
static FUNC(void, ETH_CODE) Eth_Enet_GiveBackTxBuffer (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx, VAR(Eth_BufIdxType, AUTOMATIC) u32Buf)
{
    VAR(uint32, AUTOMATIC) u32ManipulationVar;
    VAR(Eth_BufIdxType, AUTOMATIC) u32BDIdx; /* Index of linked BD (if any) */
    VAR(uint32, AUTOMATIC) u32BDAddress;

#if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNum; /* Number of buffers in current frame */
#endif /* ETH_USE_MULTIBUFFER_TX_FRAMES */

    /* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
    u32BDAddress = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);

    /* Is there a BD linked to this buffer? */
    /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
    if ( (ENET_TXB_FIRST_U8 | ENET_TXB_LINK_U8 | ENET_TXB_LOCK_U8) == Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32Buf])
    {   /* Yes, unlink the BD */
        /* Remove it on the BD side: */
        /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
        u32BDIdx = Eth_au32TxBdOfBuf[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32Buf];
        /* Read the TXBD status */
        u32ManipulationVar = Eth_Enet_ReadTxBDBitsA(u32BDAddress, u32BDIdx);
        /* Clear BufLinked flag */
        u32ManipulationVar &= (VAR(uint32, AUTOMATIC))(~ENET_TXBD_TO1_U32);
        /* Write modified bits back */
        Eth_Enet_WriteTxBDBitsA(u32BDAddress, u32BDIdx, u32ManipulationVar);
    ...
    /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
    u32BufNum = Eth_ENET_au32TxBufGroup[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32Buf];
    while(0U != u32BufNum)
    {
        /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
        Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32Buf] = 0U;
        u32BufNum--;
        u32Buf++; /* The memory must be continuous, wrap may not occur */
    }
...
*/
/* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
static FUNC(uint32, ETH_CODE) Eth_Enet_ReadTxBDBitsA (CONST(uint32, AUTOMATIC) u32Base, CONST(Eth_BufIdxType, AUTOMATIC) u32BufNum)
{
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
...
/**
* @brief Get begin index of transmit buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufStartIndex ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                          )
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx = 0U;
    (void) u8CtrlIdx; /*to avoid compiler warning*/
    
    if(u8QueueIdx == 0U)
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufIdx = (Eth_BufIdxType)ETH_CFG_TXBUF0STARTIDX(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Get begin index of receive buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufStartIndex ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                                                          ...
                                                          )
{
    ...
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufIdx = (Eth_BufIdxType)ETH_CFG_RXBUF0STARTIDX(u8CtrlIdx);
    }
...
/**
* @brief Get start address of buffer descriptor in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_GetTxBDBufStartAddress ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
...
{
    VAR(uint32, AUTOMATIC) u32BDAddr = 0UL;
    (void) u8CtrlIdx; /*to avoid compiler warning*/
    ...
    if(u8QueueIdx == 0U)
    {
        ...
        /** @violates @ref Eth_Enet_c_REF_20 MISRA rule 10.1 */
        /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
        u32BDAddr = (uint32)ETH_CFG_TXBDBUF0STARTADDRESS(u8CtrlIdx);
    }
    else 
    {
        /* fix misra warning */
                                                             ...
                                                             )
{
    ...
    if(u8QueueIdx == 0U)
    {
        ...
        /** @violates @ref Eth_Enet_c_REF_20 MISRA rule 10.1 */
        /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
        u32BDAddr = (uint32)ETH_CFG_RXBDBUF0STARTADDRESS(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Get maximum index of transmit buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufIdxMax( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                                                            ...
                                                            )
{
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdxMax = 0U;
    (void) u8CtrlIdx; /*to avoid compiler warning*/
    
    if(u8QueueIdx == 0U)
    {
        /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
        u32BufIdxMax = (Eth_BufIdxType)ETH_CFG_TXBUF0TOTAL(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Calculate index of transmit buffer in total of transmit buffer
*/
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufAbsIdx( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                        VAR(uint8, AUTOMATIC) u8QueueIdx, \
...
/**
* @brief Get maximum index of receive buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufIdxMax( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                                                            ...
                                                            )
{
    ...
    {
        /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
        u32BufIdxMax = (Eth_BufIdxType)ETH_CFG_RXBUF0TOTAL(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Get length of receive buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(uint32, ETH_CODE) Eth_Enet_GetRxBufLength ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                                                             ...
                                                             )
{
    ...
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufLength = (uint32)ETH_CFG_CTRLRXBUF0LENBYTE(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Get number of transmit buffer in queue
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetRxBufNumber ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
...
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNum = 0U;
    (void) u8CtrlIdx; /*to avoid compiler warning*/
    
    if(u8QueueIdx == 0U)
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufNum = (Eth_BufIdxType)ETH_CFG_RXBUF0TOTAL(u8CtrlIdx);
    }
    else
    {
        /* fix misra warning */
...
/**
* @brief Get queue index by transmit buffer index
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(uint8, ETH_CODE) Eth_Enet_GetQueueIndexByTxBufIdx ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
...
{
    VAR(uint8, AUTOMATIC) u8QueueIdx = 0U;

    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    VAR(Eth_BufIdxType, AUTOMATIC) u32Queue0BufIdxMax = (Eth_BufIdxType)ETH_CFG_TXBUF0TOTAL(u8CtrlIdx);

    (void) u8CtrlIdx; /*to avoid compiler warning*/

    if(u32BufIdx < (Eth_BufIdxType)u32Queue0BufIdxMax)
    {
        u8QueueIdx = 0U;
    }
    return u8QueueIdx;
...
/**
* @brief Get buffer offset in queue by transmit buffer index
*/
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
LOCAL_INLINE FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufOffset ( \
                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
...
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufOffset = 0U;
    ...
    if(u32BufIdx < (Eth_BufIdxType)u32Queue0BufIdxMax)
    {
        u32BufOffset = (Eth_BufIdxType)u32BufIdx;
    }
    return u32BufOffset;
}
/*==================================================================================================
...
*               The variable Eth_ENET_bMoreDataForTx[u8CtrlIdx][u8QueueIdx] is set whenever the new buffer
...
*               not transmitted data (Eth_ENET_bMoreDataForTx[u8CtrlIdx][u8QueueIdx] is set to TRUE)
...
*/
FUNC(void, ETH_CODE)Eth_Enet_ERR006358(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    ...
    if (((VAR(boolean, AUTOMATIC))TRUE == Eth_ENET_bMoreDataForTx[u8CtrlIdx][u8QueueIdx]) && (0U == (u32TDARRegVal & ENET_TDAR_X_DES_ACTIVE_U32)))
    { /* TX complete and still some data with (R)eady bit set in BD ring */
        ...
        /* Clean indicator */
        Eth_ENET_bMoreDataForTx[u8CtrlIdx][u8QueueIdx] = (VAR(boolean, AUTOMATIC))FALSE;
    }
}
    #endif /* STD_ON == ERR_IPV_ENET_0001 */
#endif /* ERR_IPV_ENET_0001 */
...
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNumNeed; /* Needed number of buffers */
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNumFound; /* Number of free buffers found */
#if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
    VAR(Eth_BufIdxType, AUTOMATIC) u32IdxL; /* General purpose index to be used in loops */
#endif /* ETH_USE_MULTIBUFFER_TX_FRAMES */
    VAR(uint32, AUTOMATIC) u32BDStatus;
    VAR(Eth_BufIdxType, AUTOMATIC) u32Idx;  /* General purpose index to be used in loops */
    VAR(Eth_BufIdxType, AUTOMATIC) u32SearchCnt; /* Countdown to limit search */
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32TotalBufNum = Eth_Enet_GetTxBufNumber(u8CtrlIdx, u8QueueIdx);
    VAR(uint32, AUTOMATIC) u32BufSize     = Eth_Enet_GetTxBufLength(u8CtrlIdx, u8QueueIdx);
    VAR(uint32, AUTOMATIC) u32BDAddr      = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
    VAR(Eth_BufIdxType, AUTOMATIC) u32SearchBufMaxIdx = Eth_Enet_GetTxBufIdxMax(u8CtrlIdx, u8QueueIdx);
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufAbsIdx;
    
#if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
    /* Compute needed number of buffers. Add 14B for eth header, round up */
    u32BufNumNeed = (((uint32)(*pLength) + (u32BufSize + (14U - 1U))) / u32BufSize);
    ...
    u32SearchCnt = (u32TotalBufNum + u32BufNumNeed) - 1U;
#else /* ETH_USE_MULTIBUFFER_TX_FRAMES */
    /* One buffer is always needed */
    u32BufNumNeed = 1U;
    u32SearchCnt = u32TotalBufNum;
#endif /* ETH_USE_MULTIBUFFER_TX_FRAMES */
    /* Start at Eth_LLD_u8SearchFreeTxBufFrom (to minimize fragmentation) */
    u32Idx = Eth_ENET_u32SearchTxBufFrom[u8CtrlIdx][u8QueueIdx];
    u32BufNumFound = 0U;
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
    if ((uint8)E_OK == Mcl_CacheClearMultiLines(MCL_LMEM_CACHE_PS, u32BDAddr, ETH_TX_BUF_MEM_SIZE))
    {
    #endif
  #endif
#endif
        while( ((VAR(boolean, AUTOMATIC))FALSE == bDataAreaFound) && (u32SearchCnt != 0U))
        {
            u32BufAbsIdx = Eth_Enet_GetTxBufAbsIdx(u8CtrlIdx, u8QueueIdx, u32Idx);
            /* Check buffer status, whether the buffer may be unlocked now */
            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
            if( (ENET_TXB_LOCK_U8 | ENET_TXB_LINK_U8 | ENET_TXB_FIRST_U8) == Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx]
            )
            {   /* According to buffer flags, it may be unlocked now */
                /* Now check BD status, whether Ready bit is cleared */
                /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                u32BDStatus = ENET_TXBD_R_U32 & Eth_Enet_ReadTxBDBitsA(u32BDAddr, Eth_au32TxBdOfBuf[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx]);
                if( 0U == u32BDStatus )
                {   /* Yes, the frame is in UNLOCKEDAT state and both BD and
                    buffers shall be unlocked and unlinked now */
                    Eth_Enet_GiveBackTxBuffer(u8CtrlIdx, u8QueueIdx, u32Idx);
                }
            }
            /* Check buffer status. Is the buffer free ? */
            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
            if(0U == Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx])
            {   /* Yes, it is free */
                u32BufNumFound++;
                if( u32BufNumFound == u32BufNumNeed )
                {   /* All needed buffers found */
                    bDataAreaFound = (VAR(boolean, AUTOMATIC))TRUE;
                    u32Idx++;
                    /* Next time continue search from here */
                    if( u32Idx >= u32SearchBufMaxIdx )
                    {
                        Eth_ENET_u32SearchTxBufFrom[u8CtrlIdx][u8QueueIdx] = Eth_Enet_GetTxBufStartIndex(u8CtrlIdx, u8QueueIdx);
                    }
                    else
                    {
                        Eth_ENET_u32SearchTxBufFrom[u8CtrlIdx][u8QueueIdx] = u32Idx;
                    }
                    /* Compute index of the first buffer, u32Idx is behind allocated
                    buffers now. Continuous block => no wrap => no under-loop */
                    *pBufIdx = (Eth_BufIdxType)u32Idx - u32BufNumFound;
                    /* Compute buffer pointer, skip 14 bytes for ethernet header */
                    /* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */
                    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
                        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
                    */
                    *pData = (void *)(14U + (Eth_Enet_ComputeTxBufAddr(u8CtrlIdx, u8QueueIdx, *pBufIdx)));
                    /* Return real length of buffers minus eth header length */
                    (*pLength) = (uint16)((u32BufSize * u32BufNumNeed) - 14U);             
                    u32BufAbsIdx = Eth_Enet_GetTxBufAbsIdx(u8CtrlIdx, u8QueueIdx, *pBufIdx);
                    /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                    Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx] = ENET_TXB_LOCK_U8 | ENET_TXB_FIRST_U8; /* The 1st */
#if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
                    /* Write number of buffers belonging to this frame */
                    /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                    Eth_ENET_au32TxBufGroup[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx] = u32BufNumNeed;
                    for( u32IdxL = (*pBufIdx + 1U); u32IdxL < (*pBufIdx+u32BufNumNeed); u32IdxL++ ) /* Skip the first */
                    {
                        u32BufAbsIdx = Eth_Enet_GetTxBufAbsIdx(u8CtrlIdx, u8QueueIdx, u32IdxL);
                        /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                        Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx] = ENET_TXB_LOCK_U8;
                    }
#endif /* ETH_USE_MULTIBUFFER_TX_FRAMES */
                    /* Convert physical buffer index to logical buffer index */
                    *pBufIdx = (Eth_BufIdxType)Eth_Enet_GetTxLogicalBufIdx(u8CtrlIdx, u8QueueIdx, *pBufIdx);
                }
            }
            else
            ...
            if( u32Idx >= u32SearchBufMaxIdx )
            {   /* Wrapped */
                u32Idx = Eth_Enet_GetTxBufStartIndex(u8CtrlIdx, u8QueueIdx);
                u32BufNumFound = 0U; /* Wrap => continuity broken */
            }
        }
  ...
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
        (void)Mcl_CacheFlushMultiLines(MCL_LMEM_CACHE_PS, u32BDAddr, ETH_TX_BUF_MEM_SIZE);
    }
    #endif
  #endif
#endif
...
* @param[in]     u8CtrlIdx Index of Controller to be checked
* @param[in]     u32Buf Checked buffer DA index
...
*/
FUNC(boolean, ETH_CODE) Eth_Enet_IsTxBufLockedBT (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(Eth_BufIdxType, AUTOMATIC) u32Buf)
{
    VAR(boolean, AUTOMATIC) bResult; /* Result holder */
    VAR(uint8, AUTOMATIC) u8QueueIdx = Eth_Enet_GetQueueIndexByTxBufIdx(u8CtrlIdx, u32Buf); /* Queue index */
    VAR(Eth_BufIdxType, AUTOMATIC) u32PhysBufIdx = Eth_Enet_GetTxBufOffset(u8CtrlIdx, u32Buf); /* Physical buffer index */
    /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
    if( (ENET_TXB_LOCK_U8|ENET_TXB_FIRST_U8)== Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32PhysBufIdx])
...
* @param[in]     u32BufIdx DA Index of the buffer to be transmitted
                                        ...
                                        )
{
    ...
    VAR(Eth_BufIdxType,         AUTOMATIC) u32TotalBufNum;
    VAR(uint32,         AUTOMATIC) u32BDAddr;
    VAR(uint8,          AUTOMATIC) u8QueueIdx = Eth_Enet_GetQueueIndexByTxBufIdx(u8CtrlIdx, u32BufIdx);
    VAR(Eth_BufIdxType, AUTOMATIC) u32PhysBufIdx = Eth_Enet_GetTxBufOffset(u8CtrlIdx, u32BufIdx);
    /* Local copies of configuration (optimization): */
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    /* Compiler_Warning: (diab) this should not be a warning because they are same type. 
    No potential problem here. */
    ...
    /** @violates @ref Eth_Enet_c_REF_20 MISRA rule 10.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    u32TotalBufNum = Eth_Enet_GetTxBufIdxMax(u8CtrlIdx, u8QueueIdx);
    u32BDAddr = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);

    /* Assemble frame header */
    /* Compute address of the buffer. */
    u32BufAddr = Eth_Enet_ComputeTxBufAddr(u8CtrlIdx, u8QueueIdx, (VAR(Eth_BufIdxType, AUTOMATIC))u32PhysBufIdx);
    ...
    if ((uint8)E_OK == Mcl_CacheClearMultiLines(MCL_LMEM_CACHE_PS, u32BDAddr, ETH_TX_BUF_MEM_SIZE))
    {
        ...
        /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
        Eth_au32TxBdOfBuf[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32PhysBufIdx] = Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx];
        ...
        {   /* TxConfirmation is requested, also set TxConfirmation bit */
            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
            Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32PhysBufIdx] = ENET_TXB_LOCK_U8 | ENET_TXB_FIRST_U8 | ENET_TXB_LINK_U8 | ENET_TXB_CONF_U8;
            /* Update number of pending confirmations */
            /* Enter critical section for RMW access */
            SchM_Enter_Eth_ETH_EXCLUSIVE_AREA_00();
            Eth_u32LockedTxBufCount[u8CtrlIdx][u8QueueIdx]++;
            /* Exit Exclusive area */
        ...
        {
            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
            Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32PhysBufIdx] = ENET_TXB_LOCK_U8 | ENET_TXB_FIRST_U8 | ENET_TXB_LINK_U8;
        }
        
        /* Now write the buffer descriptor */
        /* Write pointer */
        Eth_Enet_WriteTxBDBufPtr(u32BDAddr, Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx], u32BufAddr);
        ...
        if( Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx] == (u32TotalBufNum - 1U) )
        ...
        Eth_Enet_WriteTxBDBitsA(u32BDAddr, Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx], u32ManipulationVar);
        /* Reset u32BDValue */
        u32ManipulationVar = 0U;
        ...
        { 
            u32ManipulationVar = ENET_TXBD_INT_U32;
        }
        #if (STD_ON == ETH_ENABLE_OFFLOAD_CRC_IPV4)
            u32ManipulationVar |= ENET_TXBD_IINS_U32;
        #endif
        #if ( (STD_ON == ETH_ENABLE_OFFLOAD_CRC_ICMP) || (STD_ON == ETH_ENABLE_OFFLOAD_CRC_TCP) || (STD_ON == ETH_ENABLE_OFFLOAD_CRC_UDP))
            u32ManipulationVar |= ENET_TXBD_PINS_U32;
        #endif 
        #if (STD_ON == ETH_GLOBALTIME_SUPPORT)
            u32ManipulationVar |= ENET_TXBD_TS_U32;
        #endif
        /* Write FTYPE field */
        u32ManipulationVar |= (((uint32)u8QueueIdx << 20U) & (uint32)ENET_TXBD_FTYPE_U32);
        Eth_Enet_WriteTxBDBitsB(u32BDAddr, Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx], u32ManipulationVar);
        /* This BD is not active any more, increment to next (consider wrap) */
        Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx]++;
        if( Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx] >= u32TotalBufNum )
        {
            Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx] = 0U;
        }
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
        if ((uint8)E_OK == Mcl_CacheFlushMultiLines(MCL_LMEM_CACHE_PS, u32BDAddr, ETH_TX_BUF_MEM_SIZE))
        {
    #endif
  #endif
#endif
        ...
        this is the place when the Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx] is moved by the controller however
        ...
        is safer. */
            ...
            { /* Transmission in progress */
                ...
                Eth_ENET_bMoreDataForTx[u8CtrlIdx][u8QueueIdx] = (VAR(boolean, AUTOMATIC))TRUE;
            }
        #endif /* STD_ON == ERR_IPV_ENET_0001 */
    #endif /* ERR_IPV_ENET_0001 */
...
FUNC(void, ETH_CODE) Eth_Enet_ConfigureCtrl (CONST(uint8, AUTOMATIC) u8CtrlIdx)
{
...
#if (STD_ON == ETH_GLOBALTIME_SUPPORT)
    u32RegisterValue = u32RegisterValue | ENET_EIMR_TS_TIMER_U32; /* Enable ENET timer flag */
#endif
    ...
    */
    ...
    /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
    u32RegisterValue = ETH_CFG_CTRLRXBUF0LENBYTE( u8CtrlIdx);
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
    */
...
FUNC(void, ETH_CODE)  Eth_Enet_ConfigureRxBuffers(CONST(uint8, AUTOMATIC) u8CtrlIdx)
{
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufCtr;    /* Counter used to loop over the buffers */
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNum;       /* Holds configured number of buffers */
    VAR(uint32, AUTOMATIC) u32BufLength;   /* Holds configured maximal length of
                                                                one Rx buffer */
    VAR(uint32, AUTOMATIC) u32BDValue = 0U;
    VAR(uint8,  AUTOMATIC) u8QueueIdx;
    
    for(u8QueueIdx = 0U; u8QueueIdx < ETH_QUEUE_CFG; u8QueueIdx++)
    {
        /* Get the length of one Rx buffer */
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufLength = Eth_Enet_GetRxBufLength(u8CtrlIdx, 0U);
        ...
        if(u8QueueIdx == 0U)
        {
            /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
                @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1
            */
            REG_WRITE32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_MRBR_ADDR16, (VAR(uint32,AUTOMATIC))u32BufLength);  
        }
        ...
        */
        ...
        u32BufAddress = u32BdAddress + (u32BufNum * (VAR(uint32, AUTOMATIC))32U);
        /* Check whether the number of buffers is odd */
        /** @violates @ref Eth_Enet_c_REF_8 MISRA rule 13.7 */
        if(0U != (u32BufNum & 0x01U))
        {/* Yes */
            u32BufAddress += 32U;
        }
#if STD_ON == ETH_USE_MULTIBUFFER_RX_FRAMES
        ...
        Just add when the u32BufLength smaller than 1522, otherwise, the message should store in a single buffer only.
        */
        /** @violates @ref Eth_Enet_c_REF_8 MISRA rule 13.7 */
        if (1522U > u32BufLength)
        {
            u32BufAddress += (VAR(uint32, AUTOMATIC))((((1536U / 2U) + ((VAR(uint32, AUTOMATIC))u32BufLength - 1U)) / u32BufLength) * u32BufLength);
        }
#endif /* ETH_USE_RX_FRAMES_WRAP */
#endif /* ETH_USE_MULTIBUFFER_RX_FRAMES */
        /* Store the start of RX buffers for the future use */
        Eth_ENET_u32RxBufStartAddr[u8CtrlIdx][u8QueueIdx] = u32BufAddress;
        /* Write each buffer descriptor with u32BDInit value, the buffer pointer and clear the rest */
        for(u32BufCtr = 0U; u32BufCtr < u32BufNum; u32BufCtr++)
        {
            /* Write buffer descriptor control bits and length - buffer only needs
            to be marked as empty, all other fields will be updated
            during reception */
            Eth_Enet_WriteRxBDBitsA(u32BdAddress, u32BufCtr, u32BDInit);
            /* Write buffer address into descriptor */
            Eth_Enet_WriteRxBDBufPtr(u32BdAddress, u32BufCtr, u32BufAddress);
            /* Clear the rest 24 bytes of the buffer descriptor; start at offset 8U */
            u32ClearAddress = u32BdAddress + ((VAR(uint32, AUTOMATIC))32U * u32BufCtr) + 8U;
        ...
        #endif 
            Eth_Enet_WriteRxBDBitsB(u32BdAddress, u32BufCtr, u32BDValue);
            /* Compute address of the next buffer */
            u32BufAddress = u32BufAddress + u32BufLength;
        }
        /* Close the descriptor ring */
        u32BDInit = u32BDInit | ENET_RXBD_W_U32; /* This is the last descriptor, set also WRAP bit */
        Eth_Enet_WriteRxBDBitsA(u32BdAddress, (u32BufNum - 1U), u32BDInit);  /* Mark the last one */
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
        (void)Mcl_CacheFlushMultiLines(MCL_LMEM_CACHE_PS, u32BdAddress, ETH_RX_BUF_MEM_SIZE);
    #endif
  #endif
#endif
    }
...
FUNC(void, ETH_CODE) Eth_Enet_ConfigureTxBuffers(CONST(uint8, AUTOMATIC) u8CtrlIdx)
{
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufNum;       /* Holds configured number of buffers */
    VAR(uint32, AUTOMATIC) u32BufLength;   /* Holds configured maximal length
                                                             of one Tx buffer */
    VAR(uint32, AUTOMATIC) u32BDValue = 0U;
    VAR(uint8,  AUTOMATIC) u8QueueIdx;
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufAbsIdx;
    ...
    for(u8QueueIdx = 0U; u8QueueIdx < ETH_QUEUE_CFG; u8QueueIdx++)
    {
        /* Get the length of one Tx buffer */
        u32BufLength = Eth_Enet_GetTxBufLength(u8CtrlIdx, u8QueueIdx);
        /* Get the number of configured buffers */
        u32BufNum = Eth_Enet_GetTxBufNumber(u8CtrlIdx, u8QueueIdx);
        /* Compiler_Warning: (diab) this should not be a warning because they are same type. 
        No potential problem here. */
        /* Get start address of Tx buffer descriptor rings */
        u32BdAddress = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
        /* Store the value into register - select only necessary bits */
        if(u8QueueIdx == 0U)
        {
        ...
        u32BDInit = (VAR(uint32, AUTOMATIC))u32BufLength & ENET_TXBD_LENGTH_U32;
        ...
        */
        ...
        {   /* Even number of descriptors - first data pointer is already aligned to 64 bytes */
            u32BufAddress = u32BdAddress + ((VAR(uint32, AUTOMATIC))32U * u32BufNum);
        }
        else
        {   /* Odd number of descriptors - leave 32 bytes to get the first data pointer aligned to 64 bytes */
            u32BufAddress = u32BdAddress + ((VAR(uint32, AUTOMATIC))32U * u32BufNum) + 32U;
        }
        /* Store the TX buffer start address for future use */
        Eth_ENET_u32TxBufStartAddr[u8CtrlIdx][u8QueueIdx] = u32BufAddress;
        ...
        for(u32BufCtr = 0U; u32BufCtr < u32BufNum; u32BufCtr++)
        {
            ...
            before transmission */
            Eth_Enet_WriteTxBDBitsA(u32BdAddress, u32BufCtr, u32BDInit);
            /* Write buffer address into descriptor */
            Eth_Enet_WriteTxBDBufPtr(u32BdAddress, u32BufCtr, u32BufAddress);
            /* Clear the rest 24 bytes of the buffer descriptor; start at offset 8U */
            u32ClearAddress = u32BdAddress + ((VAR(uint32, AUTOMATIC))32U * u32BufCtr) + 8U;
            for(u8ClearCtr = 0U; u8ClearCtr < 6U; u8ClearCtr++)
            {
            ...
            #endif
            /* Write FTYPE field */
            u32BDValue |= (((uint32)u8QueueIdx << 20U) & (uint32)ENET_TXBD_FTYPE_U32);
            Eth_Enet_WriteTxBDBitsB(u32BdAddress, u32BufCtr, u32BDValue);
            /* Compute address of the next buffer */
            u32BufAddress = u32BufAddress + u32BufLength;
            /* Reset buffer flags */
            u32BufAbsIdx = Eth_Enet_GetTxBufAbsIdx(u8CtrlIdx, u8QueueIdx, u32BufCtr);
            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
            Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufAbsIdx] = 0U;
        }
        /* Close the descriptor ring */
        u32BDInit = u32BDInit | ENET_TXBD_W_U32; /* This is the last descriptor */
        Eth_Enet_WriteTxBDBitsA(u32BdAddress, (VAR(Eth_BufIdxType, AUTOMATIC)) (u32BufNum - 1U), u32BDInit);  /* Mark the last one */
        /* Start free buffer search from beginning */
        Eth_ENET_u32SearchTxBufFrom[u8CtrlIdx][u8QueueIdx] = Eth_Enet_GetTxBufStartIndex(u8CtrlIdx, u8QueueIdx);
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
        (void)Mcl_CacheFlushMultiLines(MCL_LMEM_CACHE_PS, u32BdAddress, ETH_TX_BUF_MEM_SIZE);
    #endif
  #endif
#endif
    }
...
FUNC(void, ETH_CODE) Eth_Enet_EnableController(CONST(uint8, AUTOMATIC) u8CtrlIdx)
{
    ...
    for(u8QueueIdx = 0U; u8QueueIdx < (uint8)ETH_QUEUE_CFG; u8QueueIdx++)
    {
        Eth_u32ActiveRxBuf[u8CtrlIdx][u8QueueIdx] = Eth_Enet_GetRxBufStartIndex(u8CtrlIdx, u8QueueIdx);
        Eth_u32ActiveTxBD[u8CtrlIdx][u8QueueIdx] = 0U;
        Eth_u32LockedTxBufCount[u8CtrlIdx][u8QueueIdx] = 0U;
    }
...
/*================================================================================================*/
/**
* @brief         Clears the RXF/RXF1/RXF2 (received frame) flag
* @param[in]     u8CtrlIdx Index of controller in which RxIRQ flag will be cleared
*/

FUNC(void, ETH_CODE) Eth_Enet_ClearRxIrqFlag(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    if (0U == u8QueueIdx)
    {
        /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
...
/*================================================================================================*/
/**
* @brief         Clears the TXF/TXF1/TXF2 (transmitted frame) flag
* @param[in]     u8CtrlIdx Index of controller in which TxIRQ flag will be cleared
*/

FUNC(void, ETH_CODE) Eth_Enet_ClearTxIrqFlag(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    if (0U == u8QueueIdx)
    {
        /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
...
/*================================================================================================*/
/**
* @brief         Checks whether the TXF/TXF1/TXF2 flag is set
...
*/
FUNC(boolean, ETH_CODE) Eth_Enet_IsTxIrqFlagSet(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    VAR(uint32, AUTOMATIC) u32RegisterValue; /* Used to read register value */
    VAR(boolean, AUTOMATIC) bReturnValue = (VAR(boolean, AUTOMATIC)) FALSE; /* Stores the result */
...
/*================================================================================================*/
/**
* @brief         Checks whether the RXF/RXF1/RXF2 flag is set
...
*/
FUNC(boolean, ETH_CODE) Eth_Enet_IsRxIrqFlagSet(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    VAR(uint32, AUTOMATIC) u32RegisterValue; /* Used to read register value */
    VAR(boolean, AUTOMATIC) bReturnValue = (VAR(boolean, AUTOMATIC)) FALSE; /* Stores the result */
...
*/
FUNC(boolean, ETH_CODE) Eth_Enet_IsTxIrqEnabled(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    VAR(uint32, AUTOMATIC) u32RegisterValue; /* Used to read register value */
    VAR(boolean, AUTOMATIC) bReturnValue = (VAR(boolean, AUTOMATIC)) FALSE; /* Stores the result */
...
* @retval        FALSE the interrupt is disabled
*/
FUNC(boolean, ETH_CODE) Eth_Enet_IsRxIrqEnabled(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    VAR(uint32, AUTOMATIC) u32RegisterValue; /* Used to read register value */
    VAR(boolean, AUTOMATIC) bReturnValue = (VAR(boolean, AUTOMATIC))FALSE; /* Stores the result */
                                            ...
                                            )
{
    ...
    */
    ...
    /* Check whether the previous code has been successful */
...
#if STD_ON == ETH_GETCOUNTVALAPI 
FUNC(void, ETH_CODE) Eth_Enet_GetCounterValues  ( \
                               VAR(uint8, AUTOMATIC) u8CtrlIdx, \
                               P2VAR(Eth_CounterType, AUTOMATIC, ETH_APPL_DATA) pCounterPtr \
                                           )
{
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->DropPktBufOverrun = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_R_MACERR_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->DropPktCrc = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_R_CRC_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->AlgnmtErr = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_R_ALIGN_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->SqeTestErr = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_T_SQE_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->SnglCollPkt = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_T_1COL_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->MultCollPkt = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_T_MCOL_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->DfrdPkt = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_T_DEF_ADDR16);
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pCounterPtr->LatCollPkt = REG_READ32(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_T_LCOL_ADDR16);
}
#endif

#if STD_ON == ETH_GET_RXSTATS_API
FUNC(void, ETH_CODE) Eth_Enet_GetRxStats ( \
                               VAR(uint8, AUTOMATIC) u8CtrlIdx, \
                               P2VAR(Eth_RxStatsType, AUTOMATIC, ETH_APPL_DATA) pRxStats \
                                           )
{
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsDropEvents = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_IEEE_R_DROP_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsOctets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_OCTETS_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_PACKETS_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsBroadcastPkts = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_BC_PKT_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsMulticastPkts = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_MC_PKT_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsCrcAlignErrors = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_CRC_ALIGN_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsUndersizePkts = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_UNDERSIZE_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsOversizePkts = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_OVERSIZE_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsFragments = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_FRAG_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsJabbers = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_JAB_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts64Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P64_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts65to127Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P65TO127_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts128to255Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P128TO255_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts256to511Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P256TO511_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts512to1023Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P512TO1023_ADDR16));
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pRxStats->RxStatsPkts1024to1518Octets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_R_P1024TO2047_ADDR16));
}
#endif

#if STD_ON == ETH_GET_TXSTATS_API
FUNC(void, ETH_CODE) Eth_Enet_GetTxStats ( \
                               VAR(uint8, AUTOMATIC) u8CtrlIdx, \
                               P2VAR(Eth_TxStatsType, AUTOMATIC, ETH_APPL_DATA) pTxStats \
                                           )
{
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */
    pTxStats->TxNumberOfOctets = REG_READ32((uint32)(Eth_u32BaseAddr[u8CtrlIdx] + ENET_RMON_T_OCTETS_ADDR16));
}
#endif

#if STD_ON == ETH_GETTXERRORCOUNTERVALUES_API
/** @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 */
FUNC(void, ETH_CODE) Eth_Enet_GetTxErrorCounterValues ( \
                       VAR(uint8, AUTOMATIC) u8CtrlIdx, \
                       P2VAR(Eth_TxErrorCounterValuesType, AUTOMATIC, ETH_APPL_DATA) pTxErrorCounterValues \
                                               )
/** @violates @ref Eth_Enet_c_REF_27 MISRA rule 8.1 */
{
    /* Prevent misra, compiler warnings */
    (void)u8CtrlIdx;
    (void)pTxErrorCounterValues;
}
#endif
...
FUNC(void, ETH_CODE) Eth_Enet_EnableEgressTimeStamp  ( \
                                VAR(uint8, AUTOMATIC) u8CtrlIdx, \
                                VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx \
                                                        )
{
    ...
    VAR(uint8, AUTOMATIC) u8QueueIdx = Eth_Enet_GetQueueIndexByTxBufIdx(u8CtrlIdx, u32BufIdx);
    /* Calculate physical buffer index */
    VAR(Eth_BufIdxType, AUTOMATIC) u32PhysBufIdx = Eth_Enet_GetTxBufOffset(u8CtrlIdx, u32BufIdx);
    /* Store buffer start address */
    VAR(uint32, AUTOMATIC) u32BdAddress = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
    
    /* Read TX buffer descriptor */
    u32BDEnhanceValue = Eth_Enet_ReadTxBDBitsB(u32BdAddress, u32PhysBufIdx);
    /* Set TS bit on TX buffer descriptor */
    u32BDEnhanceValue |= ENET_TXBD_TS_U32;
    Eth_Enet_WriteTxBDBitsB(u32BdAddress, u32PhysBufIdx, u32BDEnhanceValue);
}
                                                 ...
                                                 )
{
    ...
    if ( ENET_TXBD_TS_U32 == (u32BDEnhanceValue & (ENET_TXBD_TS_U32)) )
    {
        ...
        u32TxBdTimeStamp = Eth_Enet_ReadTxBDTimestampPtr(u32BdAddress, u32PhysBufIdx);
                                                 ...
                                                 )
{
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32CountBufIdx;
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 
        @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
    */
    /* Compiler_Warning: This is a cast from uint32 to pointer type (can be 32bit or 64 bit). There is no loss of information, no functional problem. */ 
    VAR(uint32, AUTOMATIC) u32BdAddress;
#if STD_ON == ETH_USE_RX_FRAMES_WRAP
    VAR(uint32, AUTOMATIC) u32BufLenByte;
#endif
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufTotal;

    for (u8QueueIdx = 0U; u8QueueIdx < ETH_QUEUE_CFG; u8QueueIdx++)
    {
#if STD_ON == ETH_USE_RX_FRAMES_WRAP
        u32BufLenByte = Eth_Enet_GetRxBufLength(u8CtrlIdx, u8QueueIdx);
#endif
        u32BufTotal = Eth_Enet_GetRxBufNumber(u8CtrlIdx, u8QueueIdx);
#if STD_ON == ETH_USE_RX_FRAMES_WRAP
        /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 
            @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
        */
        if (Eth_ENET_u32RxBufStartAddr[u8CtrlIdx][u8QueueIdx] < (VAR(uint32, AUTOMATIC))(pDataPtr))
        {
#endif /* ETH_USE_RX_FRAMES_WRAP */
            /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
            for (u32CountBufIdx = 0U; u32CountBufIdx <= u32BufTotal; u32CountBufIdx++)
            {
                /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
                /** @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3 */  
                ...
                if ((Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, u32CountBufIdx) + 14U) == (VAR(uint32, AUTOMATIC))(pDataPtr))
                {
                    bFoundBufIdx = (VAR(boolean, AUTOMATIC))TRUE;
                    u32BufIdx = u32CountBufIdx;
                    u32BdAddress = Eth_Enet_GetRxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
                    break;
                }
            }
        ...
        {
            for (u32CountBufIdx = u32BufTotal; u32CountBufIdx > 0U; u32CountBufIdx--)
            {
                /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 
                    @violates @ref Eth_Enet_c_REF_2 MISRA rule 11.3
                    @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1
                */
                if ( (  Eth_Enet_ComputeRxBufAddr(u8CtrlIdx, u8QueueIdx, 0U) + 14U - \
                    ((u32BufTotal - (VAR(uint32, AUTOMATIC))u32CountBufIdx) * u32BufLenByte) \
                    ) == (VAR(uint32, AUTOMATIC))(pDataPtr) \
                    )
                {
                    bFoundBufIdx = (VAR(boolean, AUTOMATIC))TRUE;
                    u32BufIdx = u32CountBufIdx;
    ...
    {
        /* Reading Time stamp from Buffer Descriptor */
        /** @violates @ref Eth_Enet_c_REF_26 MISRA rule 9.1 */
        u32RxBdTimeStamp = Eth_Enet_ReadRxBDTimestampPtr(u32BdAddress, u32BufIdx);
        /* Reading ns of current time to check whether wrap occurred before or after buffer is timestamps */
        u32CurrTimeStamp = Eth_Enet_ReadTimerValue(u8CtrlIdx);
        u32CurrSeconds = Eth_LocalTime[u8CtrlIdx].seconds;
        u16CurrSecondsHi = Eth_LocalTime[u8CtrlIdx].secondsHi;
...
* @param[in]     u8FifoIdx Specifies the related fifo
...
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufCtr; /* Buffer referencing variable */
    VAR(Eth_Enet_enRxBufferType, AUTOMATIC) eBufferType; /* Used to obtain
                                             buffer type from lower layers */
    VAR(Eth_BufIdxType, AUTOMATIC) u32MultiEnd = 0U; /* Used to obtain number of last buffer in
                                                     the multi-buffer frame */
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufCtrMax; /* Variable used to hold the buffers
    ...
    VAR(uint8,   AUTOMATIC) u8QueueIdx = u8FifoIdx;
    ...
    u32BufCtr = Eth_u32ActiveRxBuf[u8CtrlIdx][u8QueueIdx];
    /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
    u8EthIfCtrlIdx = ETH_CFG_CTRLIDXINETHIF(u8CtrlIdx); /* Get the value */
    ...
    u32BufCtrMax = Eth_Enet_GetRxBufIdxMax(u8CtrlIdx, u8QueueIdx) - 1U;
#ifdef MCAL_CACHE_RUNTIME_MNGMNT
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
    if ((uint8)E_OK == Mcl_CacheClearMultiLines(MCL_LMEM_CACHE_PS, Eth_Enet_GetRxBDBufStartAddress(u8CtrlIdx, u8QueueIdx), ETH_RX_BUF_MEM_SIZE))
    {
    #endif
  #endif
#endif
        ...
        {
            /** @violates @ref Eth_Enet_c_REF_17 MISRA rule 1.2 */
            eBufferType = Eth_Enet_GetRxBufferType(u8CtrlIdx, u8QueueIdx, u32BufCtr, u32BufCtrMax, &u32MultiEnd);
            ...
            {   /* Single buffer frames are the only to be reported */
                ...
                /** @violates @ref Eth_Enet_c_REF_17 MISRA rule 1.2 */
                Eth_Enet_GetRxBufferData(u8CtrlIdx, u8QueueIdx, u32BufCtr, u32MultiEnd, &u16Length, &pu8DataPtr, &u32FrameStatus);
                ...
                * and if is multicast check if is in the multicast pool */
                ...
                {   /* No error, frame can be passed to upper layers */
                    ...
                    {   /* Another valid received frame has already been found */
                        ...
                        Eth_u32ActiveRxBuf[u8CtrlIdx][u8QueueIdx] = u32BufCtr;
                    }
                }
                /* Clear the buffers */
                if ((VAR(boolean, AUTOMATIC))FALSE == bFinishLoop)
                {
                    ...
                    Eth_Enet_ClearRxIrqFlag(u8CtrlIdx, u8QueueIdx);
                    /* Clear all the first buffer */
                    Eth_Enet_ClearRxBuffer(u8CtrlIdx, u8QueueIdx, u32BufCtr);
                    /* Clear all others belong to frame */
                    while(u32BufCtr != u32MultiEnd)
                    {
                        u32BufCtr++;
                        if(u32BufCtr > u32BufCtrMax)
                        {
                            u32BufCtr = Eth_Enet_GetTxBufStartIndex(u8CtrlIdx, u8QueueIdx);
                        }
                        Eth_Enet_ClearRxBuffer(u8CtrlIdx, u8QueueIdx, u32BufCtr);
                    }
                    /* Continue with the next buffer */
            ...
            if ((2U * (VAR(uint32, AUTOMATIC))(u32BufCtrMax)) <= u32ProtectionCounter)
        ...
        if(RX_BUF_OVERFLOW == eBufferType)
        {
            ...
            #endif
            for(u32BufCtr = 0U; u32BufCtr <= u32BufCtrMax; u32BufCtr++)
            {
                Eth_Enet_ClearRxBuffer(u8CtrlIdx, u8QueueIdx, u32BufCtr);
            }
        }
  ...
  #if STD_ON == MCAL_CACHE_RUNTIME_MNGMNT
    #if (MCL_SYNCRONIZE_CACHE == STD_ON)
        (void)Mcl_CacheFlushMultiLines(MCL_LMEM_CACHE_PS, Eth_Enet_GetRxBDBufStartAddress(u8CtrlIdx, u8QueueIdx), ETH_RX_BUF_MEM_SIZE);
    }
    #endif
  #endif
#endif
...
{
    VAR(Eth_BufIdxType, AUTOMATIC) u32LogicalBufIdx; /* Counter used for logical buffer referencing */
    VAR(Eth_BufIdxType, AUTOMATIC) u32BufIdx; /* Counter used for physical buffer referencing */
    VAR(Eth_BufIdxType, AUTOMATIC) u32TotalBufNum; /* Variable used to hold the buffs number
    ...
    VAR(Eth_BufIdxType, AUTOMATIC) u32BDIdx; /* Index of linked BD */
    VAR(uint32, AUTOMATIC) u32BDStatus; /* Status bits of linked BD */
    VAR(uint8, AUTOMATIC) u8TxBufFlag; /* Temporary Tx buffer flag */
    VAR(Std_ReturnType, AUTOMATIC) Result = (VAR(Std_ReturnType, AUTOMATIC))E_NOT_OK;
    VAR(uint8, AUTOMATIC) u8QueueIdx;
    ...
    u32TotalBufNum = (Eth_BufIdxType)ETH_CFG_TXLOGICALBUFTOTAL(u8CtrlIdx); /* Get total number of logical buffers */
    /* Compiler_Warning: (diab) this should not be a warning because they are same type. 
    No potential problem here. */
    /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
    /** @violates @ref Eth_Enet_c_REF_7 MISRA rule 11.1 */
    ...
    Eth_Enet_ERR006358(u8CtrlIdx, u8QueueIdx);  /* Process the errata 6358 workaround */
...
#endif
        /** @violates @ref Eth_Enet_c_REF_24 MISRA rule 14.1 */
        for(u32LogicalBufIdx = 0U; u32LogicalBufIdx < u32TotalBufNum; u32LogicalBufIdx++)
        {
            u8QueueIdx = Eth_Enet_GetQueueIndexByTxBufIdx(u8CtrlIdx, u32LogicalBufIdx);
            SchM_Enter_Eth_ETH_EXCLUSIVE_AREA_01();
            if (Eth_u32LockedTxBufCount[u8CtrlIdx][u8QueueIdx] > 0U)
            {
                SchM_Exit_Eth_ETH_EXCLUSIVE_AREA_01();
                u32BufIdx = Eth_Enet_GetTxBufOffset(u8CtrlIdx, u32LogicalBufIdx);
                u32BDAddr = Eth_Enet_GetTxBDBufStartAddress(u8CtrlIdx, u8QueueIdx);
                
                SchM_Enter_Eth_ETH_EXCLUSIVE_AREA_01();
                /* At first check status bits of buffer */
                /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                u8TxBufFlag = Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufIdx];
                SchM_Exit_Eth_ETH_EXCLUSIVE_AREA_01();
                ...
                {   /* The frame is in LOCKEDAT or LOCKEDIT state */
                    ...
                    {
                        /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                        u32BDIdx = Eth_au32TxBdOfBuf[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufIdx];
                        ...
                        u32BDStatus = Eth_Enet_ReadTxBDBitsA(u32BDAddr, u32BDIdx);
                        ...
                        { /* Ready bit is zero -> frame was sent -> remove the link */
                            ...
                            SchM_Enter_Eth_ETH_EXCLUSIVE_AREA_01();
                            /** @violates @ref Eth_Enet_c_REF_23 MISRA rule 12.1 */
                            Eth_au8TxBufFlags[u8CtrlIdx][(uint32)ETHTXBUFNUM * (uint32)u8QueueIdx + u32BufIdx] =  u8TxBufFlag;
                            SchM_Exit_Eth_ETH_EXCLUSIVE_AREA_01();
                            /* Clear BufLinked flag */
                            u32BDStatus &= (VAR(uint32, AUTOMATIC))(~ENET_TXBD_TO1_U32);
                            /* Write modified bits back */
                            Eth_Enet_WriteTxBDBitsA(u32BDAddr, u32BDIdx, u32BDStatus);
                        }
                    }
                    /* Successful transmission, call EthIf_TxConfirmation with Result set to E_OK
                        and free buffers only if the link is gone */
                    if(0U == (ENET_TXB_LINK_U8 & u8TxBufFlag))
                    {
                        Result = (Std_ReturnType)E_OK;
                        /* The link was removed -> LOCKEDAT state, call confirmation */
                        /** @violates @ref Eth_Enet_c_REF_5 MISRA rule 17.4 */
                        EthIf_TxConfirmation(u8EthIfCtrlIdx, u32LogicalBufIdx, Result);
                        Eth_Enet_GiveBackTxBuffer(u8CtrlIdx, u8QueueIdx, u32BufIdx);
                    ...
                    /* Transmission failed, call EthIf_TxConfirmation with Result set to E_NOT_OK */
                    else
                    {
...
/** @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 */
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufNumber ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                )
{
    ...
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufNum = (Eth_BufIdxType)ETH_CFG_TXBUF0TOTAL(u8CtrlIdx);
    }
...
/**
* @brief Get length of transmit buffer in queue
*/
/** @violates @ref Eth_Enet_c_REF_10 MISRA rule 8.10 */
/* Compiler_Warning: Variable u8CtrlIdx is used as parameter for macros. */
FUNC(uint32, ETH_CODE) Eth_Enet_GetTxBufLength ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx
                                                )
{
    ...
    {
        /** @violates @ref Eth_Enet_c_REF_11 MISRA rule 17.4 */
        u32BufLength = (uint32)ETH_CFG_CTRLTXBUF0LENBYTE(u8CtrlIdx);
    }
