...
{
    return(Eth_Enet_ReportReception(u8CtrlIdx, u8FifoIdx, bIrq));
}
/*================================================================================================*/
/**
...
{
    Eth_Enet_EnableEgressTimeStamp(u8CtrlIdx, u32BufIdx);
}
...
{
    Eth_Enet_GetEgressTimeStamp(u8CtrlIdx, u32BufIdx, pTimeQualPtr, pTimeStampPtr );
}
...
{
    return Eth_Enet_BorrowTxBuffer(u8CtrlIdx, u8QueueIdx, pBufIdx, pData, pLength);
}
...
{
    Eth_Enet_Transmit(u8CtrlIdx, u32BufIdx, u16Type, u16Length, bConfirm, pDest
#if STD_ON == ETH_BRIDGE_SUPPORT
                           , pSource
#endif /* ETH_BRIDGE_SUPPORT  */
                     );
...
FUNC(boolean, ETH_CODE) Eth_Ipw_IsTxBufLockedBT(CONST(uint8, AUTOMATIC) u8CtrlIdx, 
    CONST(Eth_BufIdxType, AUTOMATIC) u32Buf
     )
{
    return Eth_Enet_IsTxBufLockedBT(u8CtrlIdx, u32Buf);    
 }
...
FUNC(void, ETH_CODE) Eth_Ipw_ClearRxInterruptFlag(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    Eth_Enet_ClearRxIrqFlag(u8CtrlIdx, u8QueueIdx);
}

FUNC(void, ETH_CODE) Eth_Ipw_ClearTxInterruptFlag(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    Eth_Enet_ClearTxIrqFlag(u8CtrlIdx, u8QueueIdx);
}

FUNC(boolean, ETH_CODE) Eth_Ipw_IsTxInterruptFlagSet(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    return Eth_Enet_IsTxIrqFlagSet(u8CtrlIdx, u8QueueIdx);
}

FUNC(boolean, ETH_CODE) Eth_Ipw_IsRxInterruptFlagSet(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    return Eth_Enet_IsRxIrqFlagSet(u8CtrlIdx, u8QueueIdx);
}

FUNC(boolean, ETH_CODE) Eth_Ipw_IsTxInterruptEnabled(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    return Eth_Enet_IsTxIrqEnabled(u8CtrlIdx, u8QueueIdx);
}

FUNC(boolean, ETH_CODE) Eth_Ipw_IsRxInterruptEnabled(CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx)
{
    return Eth_Enet_IsRxIrqEnabled(u8CtrlIdx, u8QueueIdx);
}


#if STD_ON == ETH_USE_MULTIBUFFER_TX_FRAMES
FUNC(Eth_BufIdxType, ETH_CODE) Eth_Ipw_GetTxBufNumber ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                )
{
    return Eth_Enet_GetTxBufNumber(u8CtrlIdx, u8QueueIdx);
}
#endif /* ETH_USE_MULTIBUFFER_TX_FRAMES */ 

FUNC(uint32, ETH_CODE) Eth_Ipw_GetTxBufLength ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx
                                                )
{
    return Eth_Enet_GetTxBufLength(u8CtrlIdx, u8QueueIdx);
}
