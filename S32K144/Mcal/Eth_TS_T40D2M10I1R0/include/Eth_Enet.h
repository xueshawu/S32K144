...
==================================================================================================*/
#include "Soc_Ips.h"                          /* for ENET IP version*/
...
/** @violates @ref Eth_Enet_h_REF_1 MISRA rule 1.4 
* @violates @ref Eth_Enet_h_REF_4 MISRA rule 5.1 */
#define ETH_AR_RELEASE_MINOR_VERSION_ENET     3
/** @violates @ref Eth_Enet_h_REF_1 MISRA rule 1.4 
* @violates @ref Eth_Enet_h_REF_4 MISRA rule 5.1 */
#define ETH_AR_RELEASE_REVISION_VERSION_ENET  1
...
#define IPV_ENET_03_00_00_03        (0x03000003UL)   /* IPV_ENET_03_00_00_03 version */
...
#if (defined(IPV_ENET) && (IPV_ENET == IPV_ENET_03_00_00_03))
#define ENET_ECR_DEFAULT_RESERVED_VALUE_U32          0xF0000000U /* Default reset value*/
#else
...
#define ENET_ATCOR_COR_U32                  0x7FFFFFFFU /* 31 bit Field wrapper for COR (Correction Counter Wrap-Around Value) */
/* ENET_ATINC_ADDR16 register bitfields */
#define ENET_ATINC_INC_CORR_U32             0x00007F00U /* 7 bit wrapper for INC_CORR (Correction Increment Value) */
#define ENET_ATINC_INC_U32                  0x0000007FU /* 7 bit wrapper for INC (Clock Period of Timestamping clock in ns) */
...
#define ENET_TXBD_FTYPE_U32  0x00F00000U /* Frame type */
...
extern VAR(volatile uint16, ETH_VAR) Eth_u16ActiveRxBuf[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG];
...
extern VAR(uint8, ETH_VAR) Eth_u8LockedTxBufCount[ETH_MAXCTRLS_SUPPORTED][ETH_QUEUE_CFG];
...
extern FUNC(void, ETH_CODE) Eth_Enet_ClearTxIrqFlag (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
extern FUNC(void, ETH_CODE) Eth_Enet_ClearRxIrqFlag (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
extern FUNC(boolean, ETH_CODE) Eth_Enet_IsTxIrqFlagSet (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
extern FUNC(boolean, ETH_CODE) Eth_Enet_IsRxIrqFlagSet (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
extern FUNC(boolean, ETH_CODE) Eth_Enet_IsTxIrqEnabled (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
extern FUNC(boolean, ETH_CODE) Eth_Enet_IsRxIrqEnabled (CONST(uint8, AUTOMATIC) u8CtrlIdx, CONST(uint8, AUTOMATIC) u8QueueIdx);
#if STD_ON == ETH_GLOBALTIME_SUPPORT
...
extern FUNC(void, ETH_CODE) Eth_Enet_EnableEgressTimeStamp  ( \
                                VAR(uint8, AUTOMATIC) u8CtrlIdx, \
...
extern FUNC(Eth_RxStatusType, ETH_CODE) Eth_Enet_ReportReception( \
                                                        CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                                                        VAR(uint8, AUTOMATIC) u8FifoIdx, \
                                                        CONST(boolean, AUTOMATIC) bIrq \
                                                                 );
extern FUNC(Eth_BufIdxType, ETH_CODE) Eth_Enet_GetTxBufNumber ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx \
                                                );

extern FUNC(uint32, ETH_CODE) Eth_Enet_GetTxBufLength ( \
                CONST(uint8, AUTOMATIC) u8CtrlIdx, \
                VAR(uint8, AUTOMATIC) u8QueueIdx
                                                );
