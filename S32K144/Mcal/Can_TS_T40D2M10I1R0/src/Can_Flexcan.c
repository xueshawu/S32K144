...
/* Check if current file and CAN_IPW header file are of the same vendor */
    ...
    #error "Can_FlexCan.c and Can_IPW.h have different vendor ids"
#endif
/* Check if current file and CAN_IPW header file are of the same module */
#if (CAN_FLEXCAN_MODULE_ID_C != CAN_IPW_MODULE_ID_H)
    #error "Can_FlexCan.c and Can_IPW.h have different module ids"
#endif
/* Check if current file and CAN_IPW header file are of the same Autosar version */
    ...
    #error "AutoSar Version Numbers of Can_FlexCan.c and Can_IPW.h are different"
#endif
/* Check if current file and CAN_IPW header file are of the same Software version */
    ...
    #error "Software Version Numbers of Can_FlexCan.c and Can_IPW.h are different"
#endif

/* Check if current file and Can.h header file are of the same vendor */
#if (CAN_FLEXCAN_VENDOR_ID_C != CAN_VENDOR_ID)
    #error "Can_FlexCan.c and Can.h have different vendor ids"
#endif
/* Check if current file and Can.h header file are of the same module */
#if (CAN_FLEXCAN_MODULE_ID_C != CAN_MODULE_ID)
    #error "Can_FlexCan.c and Can.h have different module ids"
#endif
/* Check if current file and Can.h header file are of the same Autosar version */
#if ((CAN_FLEXCAN_AR_RELEASE_MAJOR_VERSION_C    != CAN_AR_RELEASE_MAJOR_VERSION) || \
     (CAN_FLEXCAN_AR_RELEASE_MINOR_VERSION_C    != CAN_AR_RELEASE_MINOR_VERSION) || \
     (CAN_FLEXCAN_AR_RELEASE_REVISION_VERSION_C != CAN_AR_RELEASE_REVISION_VERSION))
    #error "AutoSar Version Numbers of Can_FlexCan.c and Can.h are different"
#endif
/* Check if current file and Can.h header file are of the same Software version */
#if ((CAN_FLEXCAN_SW_MAJOR_VERSION_C != CAN_SW_MAJOR_VERSION) || \
     (CAN_FLEXCAN_SW_MINOR_VERSION_C != CAN_SW_MINOR_VERSION) || \
     (CAN_FLEXCAN_SW_PATCH_VERSION_C != CAN_SW_PATCH_VERSION))
    #error "Software Version Numbers of Can_FlexCan.c and Can.h are different"
#endif
...
/* Check if current file and DET header file are of the same version */
    ...
    /* Check if current file and RegLockMacros.h file are of the same Software version */
        ...
        #error "AutoSar Version Numbers of FlexCan.c and RegLockMacros.h are different"
    #endif
   #endif
  #endif
  #ifndef CAN_TIMEOUT_AS_LOOP
...
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_SetControllerToStopMode(  VAR(uint8, AUTOMATIC) Controller,
                                                                            P2CONST(Can_ControlerDescriptorType, AUTOMATIC, CAN_APPL_CONST) pCanControlerDescriptor,
                                                                            VAR(boolean, AUTOMATIC) canif_notification_required
                                                                          );
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_SetControllerToStartMode(  VAR(uint8, AUTOMATIC) Controller,
                                                                            P2CONST(Can_ControlerDescriptorType, AUTOMATIC, CAN_APPL_CONST) pCanControlerDescriptor
                                                                          );
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_SetControllerToSleepMode(  VAR(uint8, AUTOMATIC) Controller );
static FUNC (void, CAN_CODE) Can_FlexCan_ProcessTxPoll( CONST(uint8, AUTOMATIC) controller,
                                                        CONST(uint16, AUTOMATIC) u16MBGlobalIndex,
                                                        CONST(uint32, AUTOMATIC) u32TempFlag,
                                                        CONST(uint8, AUTOMATIC) u8IflagRegIndex);

#if (CAN_RXFIFO_ENABLE == STD_ON)
static FUNC (void, CAN_CODE) Can_FlexCan_ProcessRxFifo( CONST(uint8, AUTOMATIC) controller,
                                                        CONST(uint8, AUTOMATIC) u8MbIndex,
                                                        CONST(uint32, AUTOMATIC)  u32TempFlag);
#endif

static FUNC (void, CAN_CODE) Can_FlexCan_ProcessRxNormal( CONST(uint8, AUTOMATIC) controller,
                                                          CONST(uint16, AUTOMATIC) u16MBGlobalIndex,
                                                          CONST(uint32, AUTOMATIC) u32TempFlag,
                                                          CONST(uint8, AUTOMATIC) u8IflagRegIndex );
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_UpdateMB( VAR(uint8, AUTOMATIC) u8CtrlId, VAR(uint16, AUTOMATIC) u16MBGlobalIndex ,
                                                                P2CONST(Can_PduType, AUTOMATIC, CAN_APPL_CONST) PduInfo, VAR(boolean, AUTOMATIC) bEnabledFD
                                                            );
#if (CAN_DEV_ERROR_DETECT == STD_ON)
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_ValidHth (  Can_HwHandleType Hth,
...
static FUNC (void, CAN_CODE) Can_FlexCan_InitRxFiFo( VAR(uint8, AUTOMATIC) Controller, VAR(uint8, AUTOMATIC) u8HwOffset)
{
    ...
    VAR(uint16, AUTOMATIC)             u16RxFifoTableIdIndex = 0U;
    ...
    VAR(uint16, AUTOMATIC)             u16TempIndex = 0U;
    ...
    if ( CAN_CONTROLLERCONFIG_RXFIFO_U32 == (pCanControlerDescriptor->u32Options & CAN_CONTROLLERCONFIG_RXFIFO_U32) )
    {
        ...
        u16RxFifoTableIdIndex = 0U;
        /* Calculate the index pointing to table id configuration. */
        for(u16TempIndex = 0U; u16TempIndex < ((uint16)Controller); u16TempIndex++)
        {            
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            if ( CAN_CONTROLLERCONFIG_RXFIFO_U32 == (Can_pCurrentConfig->ControlerDescriptors[u16TempIndex].u32Options & CAN_CONTROLLERCONFIG_RXFIFO_U32) )
            {
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                u16RxFifoTableIdIndex = u16RxFifoTableIdIndex + (4U*(((uint16)(Can_pCurrentConfig->ControlerDescriptors[u16TempIndex].u8RxFiFoUsedMb)) - 6U));
            }
        }
        ...
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        for ( u16TempIndex = u16RxFifoTableIdIndex; u16TempIndex < (u16RxFifoTableIdIndex + (4U * (((uint16)(Can_pCurrentConfig->ControlerDescriptors[Controller].u8RxFiFoUsedMb)) - 6U))); u16TempIndex++)
        {
            ...
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            (uint32)(((Can_pCurrentConfig->pRxFiFoTableIdConfig) + u16TempIndex)->u32TableId) );
            ...
            if ( CAN_CONTROLLERCONFIG_BCC_EN_U32 != (pCanControlerDescriptor->u32Options & CAN_CONTROLLERCONFIG_BCC_EN_U32) )
            {
              ...
              if (u8RxFifoTableId <= Can_Flexcan_aNumFilter[u8FilterIndex])
              {
                REG_WRITE32(FLEXCAN_RXIMR((u8HwOffset), (uint32)(u8RxFifoTableId)), 
                 ...
                 /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                 /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                (uint32)((((Can_pCurrentConfig->pRxFiFoTableIdConfig) + u16TempIndex))->u32TableFilterMask));
              }  
        #if (CAN_BCC_SUPPORT_ENABLE == STD_ON)
            }
        #endif
        ...
        u16MbGlobalIndex = Can_ControllerStatuses[Controller].u16MBMapping[0];
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ((boolean)TRUE != Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr[u16MbGlobalIndex].CanHwObjectUsesPolling)
        {
            ...
            if ( CAN_CONTROLLERCONFIG_OVER_EN_U32 == (pCanControlerDescriptor->u32Options & CAN_CONTROLLERCONFIG_OVER_EN_U32) )
            {
                ...
                Can_ControllerStatuses[Controller].u32MBInterruptRxMask[FLEXCAN_IMASK1_INDEX_U8] |= FLEXCAN_FIFO_OVERFLOW_INT_MASK_U32;
            }
            ...
            if ( CAN_CONTROLLERCONFIG_WARN_EN_U32 == (pCanControlerDescriptor->u32Options & CAN_CONTROLLERCONFIG_WARN_EN_U32) )
            {
                ...
                Can_ControllerStatuses[Controller].u32MBInterruptRxMask[FLEXCAN_IMASK1_INDEX_U8] |= FLEXCAN_FIFO_WARNING_INT_MASK_U32;
            }
            ...
            Can_ControllerStatuses[Controller].u32MBInterruptRxMask[FLEXCAN_IMASK1_INDEX_U8] |= FLEXCAN_FIFO_AVAILABLE_INT_MASK_U32;
        }
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        Can_ControllerStatuses[Controller].u8FirstTxMBIndex = Can_pCurrentConfig->ControlerDescriptors[Controller].u8RxFiFoUsedMb;
    }
}
#endif /* (CAN_RXFIFO_ENABLE == STD_ON) */
    ...
    for (u16MBGlobalIndex = 0U; u16MBGlobalIndex < (uint16)Can_pCurrentConfig->uCanFirstHTHIndex; u16MBGlobalIndex++)
    {
        ...
        if ( Controller == (((pCanMbConfigContainer->pMessageBufferConfigsPtr) + u16MBGlobalIndex)->u8ControllerId))
        {
               ...
               )
            {
                ...
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                if ((boolean)TRUE != pCanMbConfigContainer->pMessageBufferConfigsPtr[u16MBGlobalIndex].CanHwObjectUsesPolling)
                {
                    ...
                    Can_ControllerStatuses[Controller].u32MBInterruptRxMask[(uint8)((uint8)u8MbCtrlIndex >> FLEXCAN_MB_SHIFT5BIT_U8)] |= ((uint32)1U << (uint8)(u8MbCtrlIndex & FLEXCAN_MASK_32BITS_U32));
                }
                if (Can_ControllerStatuses[Controller].u8FirstTxMBIndex < (uint8)(u8MbCtrlIndex + (uint8)1U))
                {
        ...
        if ( Controller == (((pCanMbConfigContainer->pMessageBufferConfigsPtr) + u16MBGlobalIndex)->u8ControllerId))
        {
            ...
            if (CAN_TRANSMIT == ((pCanMbConfigContainer->pMessageBufferConfigsPtr) + u16MBGlobalIndex)->eMBType )
            {
                    ...
                    if ((boolean)TRUE != pCanMbConfigContainer->pMessageBufferConfigsPtr[u16MBGlobalIndex].CanHwObjectUsesPolling)
                    {
                        ...
                        u8MbCtrlIndex = ((pCanMbConfigContainer->pMessageBufferConfigsPtr) + u16MBGlobalIndex)->u8HWMBIndex;
                        Can_ControllerStatuses[Controller].u32MBInterruptMask[(uint8)((uint8)u8MbCtrlIndex >> FLEXCAN_MB_SHIFT5BIT_U8)] |= ((uint32)1U << (uint8)(u8MbCtrlIndex & FLEXCAN_MASK_32BITS_U32));
                        Can_ControllerStatuses[Controller].u32MBInterruptTxMask[(uint8)((uint8)u8MbCtrlIndex >> FLEXCAN_MB_SHIFT5BIT_U8)] |= ((uint32)1U << (uint8)(u8MbCtrlIndex & FLEXCAN_MASK_32BITS_U32));
                    }
                                                                          ...
                                                                          )
{
    ...
    VAR(uint32, AUTOMATIC)          u32TotalElapsedTime = 0U;
#ifndef CAN_TIMEOUT_AS_LOOP
    VAR(TickType, AUTOMATIC)        u32CurrentValue = 0U;
    VAR(TickType, AUTOMATIC)        u32ElapsedTime =0U;
#endif
    ...
    */
    if ( CAN_CS_STOPPED == Can_ControllerStatuses[Controller].ControllerState)
    {
    ...
    {
        /* Transit Controller state STARTED to STOPPED*/
        if ( CAN_CS_STARTED == Can_ControllerStatuses[Controller].ControllerState)
        {
            /*
            * (CAN263) The function Can_SetControllerMode(CAN_CS_STOPPED) shall set the bits inside the CAN hardware such that the CAN controller
            ...
            */
            if ((Std_ReturnType)E_OK == Can_FlexCan_GotoFreezeMode(u8HwOffset))
            {
                ...
                /* [SWS_CAN_00499] The CAN driver shall deactivate Pretended Networking before the CAN Controller is stopped by SetControllerMode(CAN_CS_STOPPED). */
                 /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                 /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                ...
                for ( u16MBGlobalIndex = 0U; u16MBGlobalIndex < Can_pCurrentConfig->MBConfigContainer.uMessageBufferConfigCount; u16MBGlobalIndex++)
                {
                    ...
                    #endif
                            /*
                             * (CAN282) The function Can_SetControllerMode(CAN_CS_STOPPED) shall cancel pending messages.
                 ...
                 * When Freeze mode (or Autosar equiv Stop mode) need to not participate on network -> no interrupt is required to be enabled.
                 */
        ...
        else
        {
           /* Transit Controller state SLEEP to STOPPED*/
            if (CAN_CS_SLEEP == Can_ControllerStatuses[Controller].ControllerState)
            {
    ...
    if ((Std_ReturnType)E_OK == eReturnValue)
    {
        ...
        Can_ControllerStatuses[Controller].ControllerState = CAN_CS_STOPPED;
    ...
    else
    {
        ...
        if ((Std_ReturnType)E_OK == Can_FlexCan_InitController( Controller))
        {
            ...
            do 
            {
                ...
                /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                u32TempVar = (REG_READ32( FLEXCAN_MCR(u8HwOffset)) & FLEXCAN_MCR_LPMACK_U32);
...
#if (CAN_PUBLIC_ICOM_SUPPORT == STD_ON)
        /* [SWS_CAN_00498] The CAN driver shall deactivate Pretended Networking before the CAN Controller is started by SetControllerMode(CAN_CS_STARTED) */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        ...
        if(FLEXCAN_PNET_ENABLE_U32 == (FLEXCAN_PNET_ENABLE_U32 & u32TempVar))
        {
...
#endif
            /*
            (CAN261) The function Can_SetControllerMode(CAN_CS_STARTED) shall set the hardware registers in a way that makes the CAN controller participating on the network.
            ...
            */
            ...
            /* (CAN262) The function Can_SetControllerMode(CAN_CS_STARTED) shall wait for limited time until the CAN controller is fully operational. Compare to CAN371. */
            ...
            {
                /* Set the Start State. */
                Can_ControllerStatuses[Controller].ControllerState = CAN_CS_STARTED;
                eReturnValue = (Std_ReturnType)E_OK;
            }
            else
            /* software timeout */
...
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_SetControllerToSleepMode(  VAR(uint8, AUTOMATIC) Controller )
{
    ...
    */
    ...
    else
    {
        ...
        {
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
            /*[CAN290]If the CAN HW does not support a sleep mode, the function Can_SetControllerMode(CAN_CS_SLEEP) shall set the CAN controller to the logical sleep mode*/
            /*[CAN405] This logical sleep mode shall left only, if function Can_SetControllerMode(CAN_CS_WAKEUP) is called.*/
            eReturnValue = (Std_ReturnType)E_OK;
            Can_ControllerStatuses[Controller].ControllerState = CAN_CS_SLEEP;
...
{
    /* Variable for return status. */
    VAR(Std_ReturnType, AUTOMATIC) eReturnValue = CAN_BUSY;
    ...
    VAR(uint8, AUTOMATIC)        u8DataLength = 0U;
        ...
        if ( FLEXCAN_MBCS_CODETX_U32 == (uint32)(REG_READ32( FLEXCAN_MB((u8HwOffset), (uint32)(u16MBGlobalIndex))) & (uint32)(FLEXCAN_MBCS_CODE_U32)))
        {
            ...
            if ( (NULL_PTR == PduInfo->sdu) )
            {
                ...
                if((boolean)TRUE == Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr[u16MBGlobalIndex].CanTriggerTransmitEnable)
                {
                    ...
                    /* @violates @ref Can_Flexcan_c_REF_4 Violates MISRA 2004 Required Rule 1.2, Taking address of near auto variable */
                    if((Std_ReturnType)E_OK == CanIf_TriggerTransmit(TxPduId, &PduInfoPtr))
                    {
                        /* return value is OK */
                        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4,pointer arithmetic other than array indexing used */
                        ...
                        {
                            /* get data length for triger transmit */
                    ...
                    else /* return value is NOT_OK */
                    {
                        /* [SWS_CAN_00506] If development error detection for CanDrv is enabled:
                            Can_Write() shall raise CAN_E_PARAM_POINTER and shall return E_NOT_OK if
            ...
            else /* sdu pointer is not null */
            {
                ...
                for ( u8MbDataByte = (uint8)0U; u8MbDataByte < u8DataLength; u8MbDataByte++)
                {
                    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4,pointer arithmetic other than array indexing used */
                    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            ...
            /* Calculate real data length to add padding value */
            #if (CAN_FD_MODE_ENABLE == STD_ON)
                if   (u8DataLength <= (uint8)Can_Flexcan_aDataLengthMax[0])
                {
            #endif /* (CAN_FD_MODE_ENABLE == STD_ON) */
                ...
                else
                {
                    if ((((uint32)PduInfo->id) & (uint32)CAN_FD_ID_DESCRIPTOR) == CAN_FD_ID_DESCRIPTOR )
                    {
                        ...
                        {
                            if ((u8DataLength <= (uint8)Can_Flexcan_aDataLengthMax[u8DataLengthCount]) && (u8DataLength >= (uint8)Can_Flexcan_aDataLengthMin[u8DataLengthCount]))
                            {
                                u32MbConfig |= ((uint32)((uint32)CAN_FLEXCAN_MAX_PAYLOAD8_U8 + (uint32)u8DataLengthCount) << FLEXCAN_MBCS_LENGTH_SHIFT_U32);
                ...
                if ((boolean)TRUE == bEnabledFD)
                {
                    ...
                    /* Add padding value */
                    for ( u8MbDataByte = u8DataLength; u8MbDataByte < u8RealDataLength ; u8MbDataByte++)
                    {
                        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4,pointer arithmetic other than array indexing used */
                        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                                                          ...
                                                          CONST(uint8, AUTOMATIC) u8IflagRegIndex )
{
    VAR(uint32, AUTOMATIC)          u32TotalElapsedTime = 0U;
#ifndef CAN_TIMEOUT_AS_LOOP
    VAR(TickType, AUTOMATIC)        u32CurrentValue;
    VAR(TickType, AUTOMATIC)        u32ElapsedTime;
#endif
    ...
    if(FLEXCAN_MBCS_CODERXOVRR_U32 == (u32MbConfig & FLEXCAN_MBCS_CODE_U32))
    {
        ...
        (void)Det_ReportRuntimeError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_MAIN_FUNCTION_READ, (uint8)CAN_E_DATALOST);
    }
                                                        ...
                                                        CONST(uint32, AUTOMATIC)  u32TempFlag)
{
    ...
    if ( FLEXCAN_FIFOOVERFLOW_INT_INDEX_U8 == (uint32)u8MbIndex )
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        REG_WRITE32( FLEXCAN_IFLAG1(u8HwOffset), (uint32)FLEXCAN_FIFO_OVERFLOW_INT_MASK_U32);
    }
    ...
    if ( FLEXCAN_FIFOWARNING_INT_INDEX_U8 == (uint32)u8MbIndex )
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        REG_WRITE32( FLEXCAN_IFLAG1(u8HwOffset), (uint32)FLEXCAN_FIFO_WARNING_INT_MASK_U32);
    }
    ...
    if ( FLEXCAN_FIFOFRAME_INT_INDEX_U8 == (uint32)u8MbIndex )
    {
        ...
        do
        {
            ...
            */
            ...
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            REG_WRITE32(Can_IflagImask[FLEXCAN_IMASK1_INDEX_U8][u8HwOffset].u32CanIflag, u32TempFlag);
        ...
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        } while( ((REG_READ32(Can_IflagImask[FLEXCAN_IMASK1_INDEX_U8][u8HwOffset].u32CanIflag) & u32TempFlag) != 0U) &&
...
*
* @return         A Std_ReturnType value
* @retval         E_OK if all conditions are satisfied
* @retval         E_NOT_OK if development error is occurred
*
*/
#if (CAN_DEV_ERROR_DETECT == STD_ON)
           ...
           )
        {
            /*
               (CAN218) If development error detection for the Can module is enabled: The function Can_Write shall raise the
                       error CAN_E_PARAM_DATA_LENGTH if the length is more than 8 byte.
            */
            
            (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_WRITE, (uint8)CAN_E_PARAM_DATA_LENGTH);
            eReturnValue = (Std_ReturnType)E_NOT_OK;
        }
...
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_ResetController( VAR(uint8, AUTOMATIC) u8HwOffset)
{
    ...
    if (u32TotalElapsedTime < Can_u32TicksDelayDuration)
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        REG_WRITE32(FLEXCAN_CTRL(u8HwOffset), FLEXCAN_CTRL_DEFAULT_VALUE_U32);
        /* Clear Control 2 register */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        REG_WRITE32(FLEXCAN_CTRL2(u8HwOffset), FLEXCAN_CTRL2_DEFAULT_VALUE_U32);
        /* Clear Bit Timing register */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        REG_WRITE32(FLEXCAN_CBT(u8HwOffset), FLEXCAN_CBT_DEFAULT_VALUE_U32);
        if ((((uint32)1U << u8HwOffset) & CAN_FD_CONTROLLER_SUPPORTED) != 0U)
        {
            /* Clear FD Control register */
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
            REG_WRITE32(FLEXCAN_FDCTRL(u8HwOffset), FLEXCAN_FDCTRL_DEFAULT_VALUE_U32);
            /* Clear FD Bit Timing register */
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
            REG_WRITE32(FLEXCAN_FDCBT(u8HwOffset), FLEXCAN_FDCBT_DEFAULT_VALUE_U32);
        }
        eReturnValue = (Std_ReturnType)E_OK;
    }
...
FUNC (void, CAN_CODE) Can_FlexCan_InitVariables( void )
{
    ...
    for ( u8CtrlIndex = 0U; u8CtrlIndex < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8CtrlIndex++)
    {
        ...
        Can_ControllerStatuses[u8CtrlIndex].ControllerState = CAN_CS_STOPPED;
...
*/            
    ...
    /* Do not set the Abort Enable bit of the MCR */
        ...
        if (u32TotalElapsedTime >= Can_u32TicksDelayDuration)
        {
            eResetReturn = (Std_ReturnType)E_NOT_OK;
        }
        else
        {
            Can_ControllerStatuses[Controller].ControllerState = CAN_CS_STOPPED;
...
*                 This routine is called by:
*                  - Can_DeInit() from Can.c file.
*
* @param[in]      Controller: CAN controller for which the initialization shall be done. Based on configuration order list (CanControllerId).
*
* @pre            At least one controller must be configured in the Driver structure.
* @post           De-initialization controller configuration.
*
*
*/
FUNC (void, CAN_CODE) Can_FlexCan_DeInitController( VAR(uint8, AUTOMATIC) Controller)
{
    ...
    /* Get the hardware offset of controller. */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    u8HwOffset = CanStatic_pControlerDescriptors[Controller].u8ControllerOffset;

    /* Reset Controller*/
    eReturnValue = Can_FlexCan_ResetController( u8HwOffset);
    if((Std_ReturnType)E_OK == eReturnValue)
    {
        /* Set Controller state UNINIT */
        Can_ControllerStatuses[Controller].ControllerState = CAN_CS_UNINIT;
    }
}

/*================================================================================================*/
/**
* @brief          Service obtains the error state of the CAN controller.
* @details        Service obtains the error state of the CAN controller.
*                 This routine is called by:
*                  - Can_GetControllerErrorState() from Can.c file.
*
* @param[in]      Controller: CAN controller for which the initialization shall be done. Based on configuration order list (CanControllerId).
*
* @pre            At least one controller must be configured in the Driver structure.
* @post           Return the error state of Controller
*
*
*/
FUNC (Can_ErrorStateType, CAN_CODE) Can_FlexCan_GetControllerErrorState( VAR(uint8, AUTOMATIC) Controller)
{
    ...
    VAR(uint32, AUTOMATIC)  u32ErrorState = 0U;
    /* Update Error State of Controller */
    VAR(Can_ErrorStateType, AUTOMATIC)  eCtrlErrState = CAN_ERRORSTATE_BUSOFF;

    /* Get the hardware offset of controller. */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    u8HwOffset = CanStatic_pControlerDescriptors[Controller].u8ControllerOffset;

    /* Get the value from FLTCONF field */
    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
    u32ErrorState = REG_READ32(FLEXCAN_ESR(u8HwOffset)) & FLEXCAN_ESR_FLTCONF_U32;

    switch(u32ErrorState)
    {
        case (FLEXCAN_ESR_FLTCONF_ERROR_ACTIVE_U32):
        {
            eCtrlErrState = CAN_ERRORSTATE_ACTIVE;
            break;
        }
        case (FLEXCAN_ESR_FLTCONF_ERROR_PASSIVE_U32):
        {
            eCtrlErrState = CAN_ERRORSTATE_PASSIVE;
            break;
        }
        default:
        {
                                                           ...
                                                           VAR(boolean, AUTOMATIC) canif_notification_required)
{
    ...
    */
    switch (Transition)
    {
        ...
        /*
           (CAN283) The function Can_SetControllerMode(CAN_CS_STOPPED) shall not call a cancellation notification.
        */
        case (CAN_CS_STOPPED):
        ...
        case (CAN_CS_SLEEP):
            ...
            default:
                /*
                   (CAN200) If development error detection for the Can module is enabled: if an invalid transition has been requested, the function Can_SetControllerMode shall raise the error CAN_E_TRANSITION and return E_NOT_OK.
                */
#if (CAN_DEV_ERROR_DETECT == STD_ON)
                                          ...
                                          CONST(uint8, AUTOMATIC) mbindex_end )
{
    ...
    /* Variable for determining the IFLAG register for every MB: IFLAGx */
    VAR(uint8, AUTOMATIC)            u8IflagRegIndex = 0U;
    /* Variable for storing the mask for this ISR. */
    VAR(uint32, AUTOMATIC)           u32BoundaryMask = 0U;
    
    /* Get the hardware offset for this controller. */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used*/
    u8HwOffset = CanStatic_pControlerDescriptors[controller].u8ControllerOffset;
    u8MbIndex = mbindex_start;
    for (u8IflagRegIndex = ((uint8)((uint8)mbindex_start >> FLEXCAN_MB_SHIFT5BIT_U8)); u8IflagRegIndex <= ((uint8)((uint8)mbindex_end >> FLEXCAN_MB_SHIFT5BIT_U8)); u8IflagRegIndex++)
    {
        ...
        */
        u32BoundaryMask = FLEXCAN_NO_MASK_U32;
        if (u8IflagRegIndex == ((uint32)mbindex_start >> FLEXCAN_MB_SHIFT5BIT_U8))
        {
            u32BoundaryMask &= (FLEXCAN_NO_MASK_U32 << ((uint32)mbindex_start & FLEXCAN_MASK_32BITS_U32));
        }
        if (u8IflagRegIndex == ((uint32)mbindex_end >> FLEXCAN_MB_SHIFT5BIT_U8))
        {
            u32BoundaryMask &= ~(FLEXCAN_NO_MASK_U32 << (((uint32)mbindex_end & FLEXCAN_MASK_32BITS_U32) + (uint32)1U));
        }
        /* Get the content of IFLAG register for which corresponds this MB. */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u32IflagReg = (Can_ControllerStatuses[controller].u32MBInterruptTxMask[u8IflagRegIndex]) & u32BoundaryMask;
        ...
        while ( (u32IflagReg != 0U) && (u8MbIndex <= mbindex_end) )
        {
            ...
            if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
            {
                ...
                if ((uint32)u32TempFlag == (u32ImaskReg & u32TempFlag))
                {
                    u16MbGlobalIndex = Can_ControllerStatuses[controller].u16MBMapping[u8MbIndex];
                    Can_FlexCan_ProcessTxPoll(controller, u16MbGlobalIndex, u32TempFlag, u8IflagRegIndex);
                }
                else
                {
                ...
                u32IflagReg &= ~u32TempFlag;
            }
            u8MbIndex++;
        }
        /* Update MB index for next register loop check */
        u8MbIndex = (u8IflagRegIndex + 1U) * 32U;
    }
}
...
FUNC (void, CAN_CODE) Can_FlexCan_MainFunctionWrite(void)
{
    ...
    for ( u16CtrlGlobalIndex = Can_pCurrentConfig->uCanFirstHTHIndex; u16CtrlGlobalIndex < Can_pCurrentConfig->MBConfigContainer.uMessageBufferConfigCount; u16CtrlGlobalIndex++)
    {
        ...
        if ( (FLEXCAN_NULL_OFFSET_U8 != CanStatic_pControlerDescriptors[u8CtrlId].u8ControllerOffset) && \
             ((boolean)TRUE == Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr[u16CtrlGlobalIndex].CanHwObjectUsesPolling) \
           )
        {
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            u8MbIndex = Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr[u16CtrlGlobalIndex].u8HWMBIndex;
            /* Calculate the bit in the IFLAG/IMASK register allocated to a MB */
            u32TempFlag = ((uint32)0x01U) << ((uint32)u8MbIndex & FLEXCAN_MASK_32BITS_U32);
            ...
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            u32IflagReg = REG_READ32(Can_IflagImask[u8IflagRegIndex][CanStatic_pControlerDescriptors[u8CtrlId].u8ControllerOffset].u32CanIflag);
            if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
            {
                /* Enabled & TxPoll used. Process Tx interrupt flags. */
                Can_FlexCan_ProcessTxPoll(u8CtrlId, u16CtrlGlobalIndex, u32TempFlag, u8IflagRegIndex);
            }
        }
    }
                                             ...
                                             CONST(uint8, AUTOMATIC) mbindex_end )
{
    ...
    for (u8IflagRegIndex = ((uint8)((uint8)mbindex_start >> FLEXCAN_MB_SHIFT5BIT_U8)); u8IflagRegIndex <= ((uint8)((uint8)mbindex_end >> FLEXCAN_MB_SHIFT5BIT_U8)); u8IflagRegIndex++)
    {
        ...
        */
        ...
        u32IflagReg = (Can_ControllerStatuses[controller].u32MBInterruptRxMask[u8IflagRegIndex]) & u32BoundaryMask;
        /* Get the content of IFLAG register for which corresponds this MB. */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        ...
        while ( (u32IflagReg != 0U) && (u8MbIndex <= mbindex_end) )
        {
            ...
            if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
            {
                ...
                if ((uint32)u32TempFlag == (u32ImaskReg & u32TempFlag))
                {
                    ...
                    {
                        Can_FlexCan_ProcessRxFifo(controller, u8MbIndex, u32TempFlag);
                    }
                    else
                    {
                #endif /* (CAN_RXFIFO_ENABLE == STD_ON) */
                        u16MbGlobalIndex = Can_ControllerStatuses[controller].u16MBMapping[u8MbIndex];
                        Can_FlexCan_ProcessRxNormal(controller, u16MbGlobalIndex, u32TempFlag, u8IflagRegIndex);
                #if (CAN_RXFIFO_ENABLE == STD_ON)
                    }
                #endif
                }
...
FUNC (void, CAN_CODE) Can_FlexCan_MainFunctionRead( void)
{
    ...
    for ( u16MBGlobalIndex = 0U; u16MBGlobalIndex < (uint16)Can_pCurrentConfig->uCanFirstHTHIndex; u16MBGlobalIndex++)
    {
        ...
        */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ( (FLEXCAN_NULL_OFFSET_U8 != CanStatic_pControlerDescriptors[u8CtrlId].u8ControllerOffset) && \
             ((boolean)TRUE == Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr[u16MBGlobalIndex].CanHwObjectUsesPolling) \
           )
        {
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            u8MbIndex = ((Can_pCurrentConfig->MBConfigContainer.pMessageBufferConfigsPtr) + u16MBGlobalIndex)->u8HWMBIndex;
               ...
               )
            {
                for (u8MbIndex = (uint8)FLEXCAN_FIFOFRAME_INT_INDEX_U8; u8MbIndex <= (uint8)FLEXCAN_FIFOOVERFLOW_INT_INDEX_U8; u8MbIndex++)
                {
                    ...
                    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                    u32IflagReg = REG_READ32(Can_IflagImask[FLEXCAN_IMASK1_INDEX_U8][CanStatic_pControlerDescriptors[u8CtrlId].u8ControllerOffset].u32CanIflag);
                    if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
                    {
                        Can_FlexCan_ProcessRxFifo(u8CtrlId, u8MbIndex, u32TempFlag);
                    }  
                }
            ...
            else
            {
                ...
                if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
                {
                    Can_FlexCan_ProcessRxNormal(u8CtrlId, u16MBGlobalIndex, u32TempFlag, u8IflagRegIndex);
                }  
        #if (CAN_RXFIFO_ENABLE == STD_ON)
            }
        #endif /* (CAN_RXFIFO_ENABLE == STD_ON) */
...
FUNC (void, CAN_CODE) Can_FlexCan_MainFunctionMode( void)
{
    ...
    for ( u8CtrlIndex = (uint8)0U; u8CtrlIndex < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8CtrlIndex++)
    {
        ...
        {
            if(CAN_CS_SLEEP != Can_ControllerStatuses[u8CtrlIndex].ControllerState)
            {   
                /* Get the hardware offset for this controller. */
                ...
                */
                ...
                if((FLEXCAN_MCR_NOTRDY_U32 == u32NewMcr) && (CAN_CS_STARTED == Can_ControllerStatuses[u8CtrlIndex].ControllerState))
                {
            #if (CAN_PUBLIC_ICOM_SUPPORT == STD_ON)
                    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
                    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                ...
                else if ((FLEXCAN_MCR_NOTRDY_U32 != u32NewMcr) && (CAN_CS_STOPPED == Can_ControllerStatuses[u8CtrlIndex].ControllerState))
                {

                    Can_ControllerStatuses[u8CtrlIndex].ControllerState = CAN_CS_STARTED;
