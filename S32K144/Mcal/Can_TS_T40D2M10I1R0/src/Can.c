...
#define CAN_MODULE_ID_C                      80
#define CAN_AR_RELEASE_MAJOR_VERSION_C       4
#define CAN_AR_RELEASE_MINOR_VERSION_C       3
#define CAN_AR_RELEASE_REVISION_VERSION_C    1
...
#if (CAN_MODULE_ID_C != CAN_MODULE_ID)
    #error "Can.c and Can.h have different module ids"
#endif
...
#endif
/* Check if current file and CAN header file are of the same module */
#if (CAN_MODULE_ID_C != CAN_IPW_MODULE_ID_H)
    #error "Can.c and Can_IPW.h have different module ids"
#endif
...
/* Check if current file and DET header file are of the same version */
    ...
    /* Check if current file and Std_Types.h header file are of the same version */
         ...
         (CAN_AR_RELEASE_MINOR_VERSION_C != STD_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of Can.c and Std_Types.h are different"
    #endif
  #endif
...
*/
/* @violates @ref Can_c_REF_5 Violates MISRA 2004 Required Rule 8.10, external ... could be made static */
FUNC(Std_ReturnType, CAN_CODE) Can_GetControllerMode( VAR(uint8, AUTOMATIC) Controller, P2VAR(Can_ControllerStateType, AUTOMATIC, CAN_APPL_DATA) ControllerModePtr)
{
...
#if (CAN_DEV_ERROR_DETECT == STD_ON)
    /* 
        [SWS_Can_91016]  If default error detection for the Can module is enabled: The function Can_GetControllerMode shall raise the error CAN_E_UNINIT 
                    and return E_NOT_OK if the driver is not yet initialized.
    */
    if ( CAN_READY != Can_eDriverStatus )
    {
        (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERMODE, (uint8)CAN_E_UNINIT);
    }
    else
    {
        /* 
        ...
        */
        /* The Controller ID must be within range and the selected controller must be configured for use in the configuration tool. */
        /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ( (Controller >= CanStatic_pCurrentConfig->u8ControllersConfigured) || (CAN_NULL_OFFSET == CanStatic_pControlerDescriptors[Controller].u8ControllerOffset) 
           )
        {
           (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERMODE, (uint8)CAN_E_PARAM_CONTROLLER);
        }
        else
        {
            /* 
            ...
            */
            ...
            {
                (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERMODE, (uint8)CAN_E_PARAM_POINTER);
            }
            else
            {
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
                /* Point to memory location, where the state of current controller is stored */
                *ControllerModePtr = Can_ControllerStatuses[Controller].ControllerState;
...
*
*/
/* @violates @ref Can_c_REF_7  These functions represent the API of the driver. External linkage is needed to export the API.*/
/* @violates @ref Can_c_REF_5 Violates MISRA 2004 Required Rule 8.10, external ... could be made static */
FUNC(Std_ReturnType, CAN_CODE) Can_GetControllerErrorState( VAR(uint8, AUTOMATIC) ControllerId, P2VAR(Can_ErrorStateType, AUTOMATIC, CAN_APPL_DATA) ErrorStatePtr)
{
    /* Return the result of GetControllerErrorState */
    VAR(Std_ReturnType, AUTOMATIC) eReturnValue = (Std_ReturnType)E_NOT_OK;
  
#if (CAN_DEV_ERROR_DETECT == STD_ON)  
    /* 
        [SWS_Can_91005] If default error detection for the Can module is enabled: if the module is not yet initialized, 
                the function Can_GetControllerErrorState shall raise default error CAN_E_UNINIT and return E_NOT_OK. 
    */
    if ( CAN_READY != Can_eDriverStatus )
    {
        (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERERRORSTATE, (uint8)CAN_E_UNINIT);
    }
    else
    {
        /* 
            [SWS_Can_91006] If default error detection for the Can module is enabled: if the parameter ControllerId is out of range, 
                    the function Can_GetControllerErrorState shall raise default error CAN_E_PARAM_CONTROLLER and return E_NOT_OK. 
        */
                
        /* The Controller ID must be within range and the selected controller must be configured for use in the configuration tool. */
        /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ( (ControllerId >= CanStatic_pCurrentConfig->u8ControllersConfigured) || (CAN_NULL_OFFSET == CanStatic_pControlerDescriptors[ControllerId].u8ControllerOffset) 
           )
        {
           (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERERRORSTATE, (uint8)CAN_E_PARAM_CONTROLLER);
        }
        else
        {
            /* 
            ...
            */
            ...
            {
                (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_GETCONTROLLERERRORSTATE, (uint8)CAN_E_PARAM_POINTER);
            }
            else
            {
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
                *ErrorStatePtr = Can_IPW_GetControllerErrorState(ControllerId);
...
FUNC(void, CAN_CODE) Can_DeInit(void)
{
    ...
    VAR(uint8, AUTOMATIC) u8CanCtrlIdx = 0U;
    
#if (CAN_DEV_ERROR_DETECT == STD_ON)
    /* The flag of checking of Can Controller status */
    VAR(boolean, AUTOMATIC) bIsCanCtrlBusy = (boolean)FALSE;
#endif    
#if (CAN_DEV_ERROR_DETECT == STD_ON)
    if ( CAN_READY != Can_eDriverStatus )
    {
    ...
    else
    {
        ...
        for ( u8CanCtrlIdx = 0U; u8CanCtrlIdx < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8CanCtrlIdx++)
        {
            /* Skipping controllers that are not enabled. */
            /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            if ( CAN_NULL_OFFSET != (CanStatic_pControlerDescriptors[u8CanCtrlIdx].u8ControllerOffset))
            {
                if (CAN_CS_STARTED == Can_ControllerStatuses[u8CanCtrlIdx].ControllerState)
                {
                    /* 
        ...
        if ((boolean)TRUE != bIsCanCtrlBusy)
        {
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */       
            /* Loop through all Can controllers configured based CanControllerId parameter. */
            for ( u8CanCtrlIdx = 0U; u8CanCtrlIdx < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8CanCtrlIdx++)
            {
                ...
                if ( CAN_NULL_OFFSET != (CanStatic_pControlerDescriptors[u8CanCtrlIdx].u8ControllerOffset))
                {
                    ...
                    Can_IPW_DeInitController(u8CanCtrlIdx);
                }
            }
            /* Update state of Can driver */
            Can_eDriverStatus = CAN_UNINIT;
#if (CAN_DEV_ERROR_DETECT == STD_ON)        
        }
    }
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
...
* @api
*/
FUNC(Std_ReturnType, CAN_CODE) Can_SetControllerMode( VAR(uint8, AUTOMATIC) Controller,
                                                      VAR(Can_ControllerStateType, AUTOMATIC) Transition 
                                                    )
{
    /* Returns the result of transition mode. */
/* @violates @ref Can_c_REF_9 Violates MISRA 2004 Required Rule 10.1 , Prohibited Implicit Conversion: Non-constant argument to function, Implicit conversion of integer to smaller type */
    VAR(Std_ReturnType, AUTOMATIC)  eReturnValue = E_NOT_OK;
    ...
    */
    ...
    if ( CAN_UNINIT == Can_eDriverStatus )
    {
        /*
           (CAN198) If development error detection for the Can module is enabled: if the module is not yet initialized, the
                   function Can_SetControllerMode shall raise development error CAN_E_UNINIT and return E_NOT_OK.
        */
    ...
    else
    {
        ...
        else
        {
            ...
            /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            /* @violates @ref Can_c_REF_9 Violates MISRA 2004 Required Rule 10.1 , Prohibited Implicit Conversion: Non-constant argument to function, Implicit conversion of integer to smaller type */
            eReturnValue = (VAR(Std_ReturnType, AUTOMATIC))Can_IPW_SetControllerMode( Controller, &(Can_pCurrentConfig->ControlerDescriptors[Controller]), Transition,(boolean)TRUE);
...
* @api
*/
FUNC(Std_ReturnType, CAN_CODE) Can_Write( VAR(Can_HwHandleType, AUTOMATIC) Hth,
...
/* @violates @ref Can_c_REF_9 Violates MISRA 2004 Required Rule 10.1 , Prohibited Implicit Conversion: Non-constant argument to function, Implicit conversion of integer to smaller type */
    ...
    else
    {
        ...
        else
        {
            ...
            eReturnValue = (Std_ReturnType)Can_IPW_Write(Hth,PduInfo);
            ...
            */
               ...
               )
            {
                ...
                if ( (uint8)E_OK == Can_IPW_CheckBusOff(can_controller) )
                {
                    ...
                    /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                    /* @violates @ref Can_c_REF_9 Violates MISRA 2004 Required Rule 10.1 , Prohibited Implicit Conversion: Non-constant argument to function, Implicit conversion of integer to smaller type */
                    if ( E_OK == (VAR(Std_ReturnType, AUTOMATIC))Can_IPW_SetControllerMode( can_controller,  &(Can_pCurrentConfig->ControlerDescriptors[can_controller]), CAN_CS_STOPPED,(boolean)FALSE) )
                    {
...
FUNC(Std_ReturnType, CAN_CODE) Can_SetClockMode( VAR(uint8, AUTOMATIC) can_controller, VAR(Can_ClockModeType, AUTOMATIC) can_clk_mode)
{
    ...
    else
    {
        ...
        {
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
            if ( CAN_CS_STOPPED == Can_ControllerStatuses[can_controller].ControllerState )
            {
